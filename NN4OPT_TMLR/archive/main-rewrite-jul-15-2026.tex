...
Throughout this paper, we denote scalars with unbolded lower-case letters $z$, vectors (and vector-valued functions) with bolded lower-case letters $\mathbf{z}$, and matrices with unbolded capital letters $Z$. We use $\| \cdot \|$ to denote the Euclidean norm and $| \cdot |$ to denote the absolute value norm. We use $\mathbb{E}_\xi[\, \cdot \,]$ or $\mathbb{P}_\xi(\cdot)$ to denote expected value or event probability with respect to the distribution of a random variable $\xi$; if the context is obvious, we occasionally drop the dependence on $\xi$ from the subscript.
...
Fast, approximate solvers have been developed for many non-convex problems, but these often lack guarantees of finding globally optimal solutions. Our goal in this work is to supplement fast, approximate solvers with \textit{optimality certificates} obtained from deep neural networks trained to predict the optimal value $v(\bm{\theta})$ of a parametric non-convex problem. The neural network predictions may be compared with the objective value of any feasible solution $\hat{\bm{x}} \in \mathcal{C}(\bm{\theta})$ to estimate its optimality gap and thus \textit{certify} whether $\hat{\bm{x}}$ is near-globally optimal. Our framework exploits convexification techniques and conformal prediction theory to provide probabilistic guarantees on the quality of a given solution and the accuracy of our certificates.
...
Split conformal calibration \citep{vovk2005algorithmic, lei2018distribution} is a procedure for calibrating a machine learning model so that its predictions satisfy rigorous, probabilistic performance guarantees. Abstractly, given a target quantity $y(\bm{\theta})$ and any pre-trained point predictor $\hat{y}(\bm{\theta})$, the goal is to construct a \textit{prediction set} $C_\alpha(\bm{\theta}) \subseteq \mathbb{R}$ that contains the true value with a user-specified probability,
\begin{align*}
    \mathbb{P}\big[\, y(\bm{\theta}) \in C_\alpha(\bm{\theta}) \,\big] ~\geq~ 1 - \alpha,
\end{align*}
where $\alpha \in (0, 1)$ is a target miscoverage level and the probability is taken over the joint draw of the calibration sample and the test instance $\bm{\theta}$.

Remarkably, this coverage guarantee holds \textit{in finite samples}, with no assumptions about the model class, the input distribution, or the smoothness of the underlying function; for this reason conformal prediction is often called \textit{distribution-free uncertainty quantification} \citep{angelopoulos2023conformal}. This is precisely what allows us to provide probabilistic guarantees for the error distribution of deep neural networks, which are otherwise intractable to characterize analytically: rather than modeling the network's errors, split conformal calibration treats the network as a black box and calibrates around its observed residuals.

In the context of our framework, this allows us to give probabilistic guarantees on the accuracy of the neural network optimality certificates. In particular, we may control the \textit{false positive rate}---the rate at which we misclassify a suboptimal solution as globally optimal. Such control is essential: an approximate optimality certificate is meaningless without some accompanying probabilistic guarantee on how often it may err. Moreover, unlike most conformal prediction methods, which construct two-sided intervals for uncertainty quantification, our framework requires only a \textit{one-sided} interval: to certify optimality we need only a calibrated lower bound $\hat{v}(\bm{\theta}) - q_\alpha$ on the relaxation value $v_r(\bm{\theta})$, against which a feasible solution's cost is compared.

The procedure itself is simple, and captures the intuitive notion that a model's performance on a held-out data set should be indicative of its expected performance on new test instances drawn from a similar distribution. Given a predictor $\hat{v}(\cdot)$ and a held-out calibration sample $\{(\bm{\theta}_i, v_r(\bm{\theta}_i))\}_{i \in \mathcal{I}}$ that the predictor did not see during training, one computes nonconformity scores (for our scalar regression target, the residuals $\hat{v}(\bm{\theta}_i) - v_r(\bm{\theta}_i)$) and forms the calibrated bound from an appropriate empirical quantile of these scores. The exact order statistic used is what carries the finite-sample guarantee; we give the precise one-sided construction in Section~\ref{sec:experiments}.

The guarantee rests on one key assumption: the held-out data used to calibrate the model must be \textit{exchangeable} with the new test instances---informally, they must be drawn from the same probability distribution. This assumption has important implications for the validity of our framework's probabilistic guarantees in practice, particularly under distribution shift or in genuinely time-varying deployment, which we return to in Section~\ref{sec:discussion}.
...
Solving a convex relaxation provides a lower bound on the otherwise intractable optimal value $v_r(\bm{\theta}) \leq v(\bm{\theta})$ for any $\bm{\theta} \in \Theta$. The objective value of a candidate solution $\hat{\mathbf{x}} \in \mathcal{C}(\bm{\theta})$ may be compared against $v_r(\bm{\theta})$ to upper bound its \textit{optimality gap}, i.e. the gap between the solution's objective value and the optimal value:
\begin{align*}
...
\begin{tcolorbox}[
    ...
    left=10pt, right=10pt,        % Inner text padding
    top=10pt, bottom=10pt,
    grow to left by=-7pt,        % Tighter left indent
    grow to right by=-7pt,       % Tighter right indent
    colback=blue!8,               % Ultra-light blue background
    colframe=black,               % Black border line
    sharp corners,                % Keeps the box rectangular
    boxrule=0.4pt                 % Standard thin border thickness
]

\begin{center}
    \large
    \textbf{Neural Network-Based Optimality Certificates}
\end{center}
...
We solve each problem instance in the test set with IPOPT, a local interior point solver, from a random fixed initialization \citep{wachter2006implementation}. For each feasible local solution $\hat{\mathbf{x}}$, we certify optimality if $f(\hat{\mathbf{x}}\,;\bm{\theta}) \leq \hat{v}(\bm{\theta}) - \tau$.
...
\centering
\footnotesize
\caption{AC-OPF test-set statistics, SOCP relaxation. ``Relax.\ gap'' is $f(\hat{\mathbf{x}}\,;\bm{\theta}) - v_r(\bm{\theta})$ (mean and $5$th--$95$th percentile range over the test set); ``\% infeasible'' is the share of test instances for which either the relaxation or local solver failed.}
\begin{tabular}{lcccc}
\toprule
...
\texttt{case89pegase}   & $89$   & $7274\ [5113,\,9483]$          & $6.48\ [3.24,\,11.0]$     & $4.5$ \\
\texttt{case118}        & $118$  & $113200\ [70640,\,158900]$     & $274\ [140,\,368]$        & $0.0$ \\
\texttt{case300}        & $300$  & $586100\ [395600,\,810300]$    & $1091\ [634,\,1462]$      & $19.7$ \\
\texttt{case1354pegase} & $1354$ & $67390\ [47000,\,88140]$       & $49.3\ [29.0,\,75.6]$     & $0.4$ \\
\texttt{case2869pegase} & $2869$ & $126500\ [86580,\,163500]$     & $260\ [57.2,\,1606]$      & $35.7$ \\
\bottomrule
\end{tabular}
...
\texttt{case89pegase}   & $2.52\ [0.03,\,13.4]$    & $3.36$  & $7.94$  & $19.4$  \\
\texttt{case118}        & $27.8\ [0.74,\,144]$     & $21.4$  & $63.7$  & $270$   \\
\texttt{case300}        & $267\ [8.2,\,1179]$      & $478$   & $999$   & $2325$  \\
\texttt{case1354pegase} & $4.17\ [0.19,\,13.0]$    & $6.79$  & $10.2$  & $18.4$  \\
\texttt{case2869pegase} & $7.98\ [0.34,\,24.8]$    & $13.6$  & $20.0$  & $33.2$  \\
\bottomrule
\end{tabular}
...
\centering
\footnotesize
\caption{AC-OPF certification results at $\delta = 0.1\%$, SOCP relaxation, averaged over the four folds (counts out of $5000$ test instances, fewer where infeasible instances were dropped; see Table~\ref{tab:ACOPF-data-socp}).}
\begin{tabular}{lc cccc}
\toprule
...
\multirow{3}{*}{\texttt{case39}} & $90\%$ & $966$ & $0$ & $124$ & $3560$ \\
 ...
 & $99\%$ & $61$  & $0$ & $124$ & $4466$ \\
\midrule
\multirow{3}{*}{\texttt{case89pegase}} & $90\%$ & $538$ & $44$ & $1021$ & $3173$ \\
 & $95\%$ & $269$ & $20$ & $1044$ & $3442$ \\
 & $99\%$ & $56$  & $4$  & $1061$ & $3654$ \\
\midrule
...
\multirow{3}{*}{\texttt{case300}} & $90\%$ & $0$ & $160$ & $3856$ & $0$ \\
 ...
 & $99\%$ & $0$ & $12$  & $4003$ & $0$ \\
\midrule
\multirow{3}{*}{\texttt{case1354pegase}} & $90\%$ & $4717$ & $0$ & $2$ & $260$ \\
 & $95\%$ & $4546$ & $0$ & $2$ & $432$ \\
 & $99\%$ & $2680$ & $0$ & $2$ & $2297$ \\
\midrule
\multirow{3}{*}{\texttt{case2869pegase}} & $90\%$ & $2475$ & $9$ & $358$ & $374$ \\
 & $95\%$ & $2203$ & $4$ & $363$ & $646$ \\
 & $99\%$ & $816$  & $1$ & $366$ & $2034$ \\
\bottomrule
\end{tabular}
...
\paragraph{Results.} Tables~\ref{tab:ACOPF-data-socp}--\ref{tab:ACOPF-cert-chordal} report the results. Tables~\ref{tab:ACOPF-data-socp}--\ref{tab:ACOPF-data-chordal} show that the chordal SDP relaxation is tighter than the SOCP relaxation on every test system---for example, on \texttt{case118} the mean relaxation gap falls from $274$ (SOCP) to $2.0$ (chordal), and on \texttt{case1354pegase} from $49.3$ to $6.0$---at the cost of being intractable at our time budget for the largest system (\texttt{case2869pegase}). The gap between the two relaxations is small on several systems, however: on \texttt{case89pegase} and \texttt{case1354pegase} the SOCP relaxation is already tight relative to the mean optimal cost (mean relative gap $0.09\%$ and $0.07\%$ respectively), which, as we discuss below, is what enables certification under the cheaper relaxation. The infeasibility rate (instances where either the relaxation or local solver failed) ranges from $0\%$ on the small systems to as high as $36\%$ on \texttt{case2869pegase}, reflecting the increasing numerical difficulty of both problems at scale.
...
The certification behavior in Tables~\ref{tab:ACOPF-cert-socp}--\ref{tab:ACOPF-cert-chordal} is governed by the tightness of the relaxation relative to the target tolerance $\delta = 0.1\%$. Under the chordal SDP, which is tight for every system, true positives dominate at every confidence level, e.g.\ $3846$ out of $3855$ instances certified on \texttt{case1354pegase} at $95\%$ confidence, with zero false positives throughout. Under the SOCP relaxation the outcome splits cleanly by whether the relaxation gap falls below $\delta$: on systems where it does---\texttt{case89pegase}, \texttt{case1354pegase}, and \texttt{case2869pegase}, whose mean relative gaps are all under $0.1\%$---the SOCP certifies a large share of instances (e.g.\ $4546$ certified on \texttt{case1354pegase} at $95\%$ confidence, with zero false positives), whereas on \texttt{case118} and \texttt{case300}, where the relaxation gap exceeds $\delta$ for essentially every instance, there are zero true positives and only the small false-positive counts the conformal guarantee permits at rate $1-\alpha$ (e.g.\ $74$ out of $5000$ on \texttt{case118} at $90\%$ confidence). Where the relaxation gap sits just below $\delta$ the certificate is sound but conservative: on \texttt{case89pegase} (mean relative gap $0.09\%$) the conformal offset leaves little room, so only $538$ of the $3711$ $\delta$-optimal instances are certified at $90\%$, compared with the more comfortable margin on \texttt{case1354pegase} ($0.07\%$). Across every configuration the false-positive rate stays at or below its nominal level $1-\alpha$, exactly as the split-conformal guarantee predicts. This illustrates a central property of our framework: the quality of the certificate is inherited from the quality of the underlying relaxation, and a tighter relaxation (higher in a convexification hierarchy) directly enables certification at a stricter optimality tolerance.
...
Our framework's central appeal is that it decouples the cost of \emph{obtaining} a
global optimality guarantee from the cost of \emph{using} one. The expensive
convex relaxation is solved only offline, and at deployment a single network
evaluation certifies any fast solver's solution---local search, a heuristic, or a
learned solution map---supplying the optimality guarantee those methods lack
rather than replacing them. Because the learning target is the scalar value
function $v_r(\bm{\theta})$ rather than a high-dimensional, often discontinuous
solution map, it is a comparatively easy quantity to approximate, and the same
learned surrogate serves every downstream solver.

The probabilistic guarantee rests on \emph{exchangeability} between the
calibration sample and the test instance. This holds when parameters are drawn
i.i.d.\ from a fixed distribution, as in our experiments. In genuine time-varying
deployment---where $\bm{\theta}_t$ is temporally correlated or subject to
distribution shift---marginal coverage can degrade, and the calibrated offset
should be refreshed online or replaced by a shift-robust (e.g.\ weighted)
conformal variant. A practical corollary is that the offset must be estimated on
data exchangeable with the test stream: residuals measured on the \emph{training}
set are optimistic, since the network fits its training data more closely than
fresh instances, and would understate the true error. Split conformal calibration
on a held-out fold, as we use throughout, avoids this.

Two further limitations bound what the certificate can claim. First, its coverage
...
calibrates a learned quantile model rather than a single global offset, is a
natural route to approximate conditional coverage and an interesting direction for
future work. Second, the certificate's \emph{power} is inherited from the
relaxation: as our analysis shows, the miss rate is governed by the relaxation gap
$v(\bm{\theta}) - v_r(\bm{\theta})$, so a loose relaxation yields a sound but
conservative certificate. Tightening the relaxation---moving up a convexification
hierarchy---directly buys certification at a stricter optimality tolerance, at the
offline cost of a larger convex program.

Finally, reporting performance as an ROC curve, rather than at a single fixed
$(\delta, \alpha)$, reflects a genuine feature of the framework. A single decision
threshold corresponds to an entire family of $(\delta, \alpha)$ optimality claims:
for the same threshold one may assert a tighter optimality gap with lower
confidence, or a looser gap with higher confidence. Rather than an ambiguity to be
resolved once, this is a flexibility to be exploited at deployment---one calibrated
model exposes the whole speed/accuracy/confidence frontier, and the operator
selects the operating point appropriate to each decision without retraining or
recalibrating.
