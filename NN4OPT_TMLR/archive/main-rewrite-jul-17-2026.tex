...
It is a non-convex QCQP parameterized by the real and reactive load $\bm{\theta} = (P_D, Q_D)$ at each bus.

% NOTE: full AC-OPF problem formulation to be written here (left to co-author).

For each test system we sample real and reactive load profiles $\bm{\theta} = (P_D, Q_D)$ and label them with two convex relaxations that lower-bound the true optimum: the second-order cone (SOCP / Jabr) relaxation, and the tighter chordal semidefinite (SDP / Lavaei-Low) relaxation.
(CLAUDE to describe sampling procedure in greater depth)
Feasible solutions are obtained with an interior point method (IPOPT). The two relaxations trade off tightness against cost---the SOCP is cheap, the chordal SDP tight but far slower (Table~\ref{tab:timing})---and because the true optimum $v(\bm{\theta})$ is intractable, we treat the chordal SDP value as ground truth (and, for \texttt{case2869pegase}, whose chordal relaxation is intractable at our time budget, the SOCP value). We study eight standard IEEE and PEGASE systems from $9$ to $2{,}869$ buses.

(CLAUDE to add table with description of cases: case name, number of buses, number of lines, number of generators, average load, min/max load)
...
Here, we specifically consider \textit{ellipsoidal} uncertainty sets parameterized by the center and principal axes $\bm{\theta} = (\bm{\mu}, \bm{\sigma}) \in \mathbb{R}^{2n}$,
\begin{align*}
    \mathcal{U}(\bm{\theta}) ~=~ \{ \bm{\mu} + \mathrm{diag}(\bm{\sigma}) \, \bm{\zeta} : \| \bm{\zeta} \| \leq \rho \},
\end{align*}
...
\label{prob:knapsack}
\end{equation}
Unlike other experiments, we do not seek a convex relaxation for problem \cref{prob:knapsack}{}{}. That is because this problem admits an exact reformulation as a mixed-integer second-order cone program (MISOCP):
\begin{equation}
\begin{aligned}
    v(\bm{\theta}) ~=~ \max_{x} \quad & \mathbf{c}^\top \mathbf{x} \\
    \text{s.t.} \quad & \bm{\mu}^\top \mathbf{x} + \rho\,\|\mathrm{diag}(\bm{\sigma})\,\mathbf{x}\| \leq b
\end{aligned}
...
In our experiment, we consider robust 0-1 knapsack with $n=50$ items.
The uncertainty set parameters $\mathbf{\theta}$ are drawn from a distribution (CLAUDE to describe sampling distribution for this experiment).
For each training instance, we solve problem \cref{prob:knapsack-socp}{}{} to certified global optimality with a branch-and-bound solver from Gurobi \citep{gurobi}. 
The fast solver we certify is a greedy heuristic algorithm that ranks and selects items based on the ratio of value to weight plus uncertainty (CLAUDE to verify or improve description of greedy heuristic for this experiment).
Because the value function of a combinatorial problem is comparatively hard to learn, we re-ran this experiment with $80{,}000$ and $640,000$ training instances.
...
Tables~\ref{tab:offline}--\ref{tab:timing} report all results at the fixed operating point $\delta = 3\% \times\mathrm{std.\,}v(\bm{\theta})$ and $\alpha = 0.01$. Table~\ref{tab:offline} covers the offline stages: the mean and spread of the (relaxed) optimal value, the relaxation gap, the mean absolute error of the trained neural network predictions, and the calibrated conform offset $\hat{q}_{99}$. Table~\ref{tab:online} reports the deployment results: the local solver's optimality gap and the certification counts. Across every experiment the false positive rate is near-zero or close to the $1\%$ target, while the number of certified optima (TP) is high wherever the network is accurate enough that $\hat{q}_{99}$ fits within $\delta$ (AC-OPF, inverse kinematics, QCQP), and low where it is not (MIMO detection, and to a lesser extent robust knapsack), whose rougher value functions the network fits less precisely. Table~\ref{tab:timing} shows the offline solve cost: higher-order relaxations are consistently, and often dramatically, slower (e.g.\ the chordal SDP is $50$--$130\times$ slower than the SOCP on AC-OPF), whereas a network prediction is sub-microsecond.
...
Mean $v^\ast$ & Std $v^\ast$ & Relax.\ Gap$^\ast$ & MAE & $q_{99}$ \\
\midrule
  \multirow{1}{*}{QCQP} & Shor &  & 2.65 & 4.63 & 0 & 0.0022 & 0.012 \\
\midrule
  \multirow{2}{*}{Inverse Kinematics} & Shor &  & 0.036 & 0.094 & 0.0029 & 7.7e-05 & 0.00055 \\
   & Lasserre-2 &  & 0.039 & 0.094 & -4e-08 & 8.4e-05 & 0.00053 \\
\midrule
  \multirow{1}{*}{MIMO Detection} & Shor &  & 0.79 & 0.41 & 0.00045 & 0.017 & 0.057 \\
\midrule
  \multirow{15}{*}{AC-OPF} & SOCP & \texttt{case9} & 4,692 & 1,452 & 0.04 & 0.72 & 2.79 \\
   & SOCP & \texttt{case14} & 7,049 & 1,906 & 5.8 & 2.82 & 17.5 \\
   & SOCP & \texttt{case39} & 33,515 & 12,532 & 6.6 & 33.2 & 219 \\
   & SOCP & \texttt{case89pegase} & 7,274 & 1,410 & 6.46 & 2.52 & 19.4 \\
   & SOCP & \texttt{case118} & 113,180 & 28,637 & 272 & 27.8 & 270 \\
   & SOCP & \texttt{case300} & 586,141 & 133,013 & 803 & 267 & 2,325 \\
   & SOCP & \texttt{case1354pegase} & 67,385 & 13,340 & 43.7 & 4.17 & 18.4 \\
   & SOCP & \texttt{case2869pegase} & 126,542 & 24,379 & 0 & 7.98 & 33.2 \\
   & SDP & \texttt{case9} & 4,692 & 1,452 & 0 & 0.73 & 2.71 \\
   & SDP & \texttt{case14} & 7,054 & 1,907 & 0 & 2.94 & 18.2 \\
   & SDP & \texttt{case39} & 33,522 & 12,533 & 0 & 30.8 & 196 \\
   & SDP & \texttt{case89pegase} & 7,281 & 1,412 & 0 & 2.52 & 19.1 \\
   & SDP & \texttt{case118} & 113,452 & 28,705 & 0 & 28.4 & 281 \\
   & SDP & \texttt{case300} & 583,523 & 130,830 & 0 & 237 & 1,485 \\
   & SDP & \texttt{case1354pegase} & 67,676 & 13,279 & 0 & 4.84 & 19.4 \\
\midrule
  \multirow{2}{*}{Robust Knapsack} & None & \texttt{n=20000} & 28.8 & 1.15 & -- & 0.19 & 0.57 \\
   & None & \texttt{n=80000} & 28.8 & 1.13 & -- & 0.14 & 0.44 \\
\bottomrule
\end{tabular}
\end{table}
...
$\alpha = 1\%$ (offset $q_{99}$), averaged over the four folds. ${}^\ast$Optimality gap uses
...
\midrule
  \multirow{1}{*}{QCQP} & Shor &  & 9.76 & 71.1 & 2524 & 0 & 2474 & 2 \\
\midrule
  \multirow{2}{*}{Inverse Kinematics} & Shor &  & 0.011 & 2 & 4533 & 0 & 193 & 267 \\
   & Lasserre-2 &  & 0.011 & 2 & 4799 & 0 & 193 & 1 \\
\midrule
  \multirow{1}{*}{MIMO Detection} & Shor &  & 0.0019 & 2.64 & 96 & 0 & 4 & 4900 \\
\midrule
  \multirow{15}{*}{AC-OPF} & SOCP & \texttt{case9} & 4e-05 & 0.0014 & 5000 & 0 & 0 & 0 \\
   & SOCP & \texttt{case14} & 0.0057 & 5.15 & 4999 & 0 & 0 & 1 \\
   & SOCP & \texttt{case39} & 4.11 & 351 & 4542 & 0 & 0 & 109 \\
   & SOCP & \texttt{case89pegase} & 0.026 & 0.33 & 4698 & 0 & 0 & 75 \\
   & SOCP & \texttt{case118} & 2.01 & 4.82 & 4952 & 0 & 0 & 48 \\
   & SOCP & \texttt{case300} & 235 & 9,692 & 3702 & 0 & 9 & 232 \\
   & SOCP & \texttt{case1354pegase} & 5.96 & 32.9 & 3844 & 0 & 0 & 0 \\
   & SOCP & \texttt{case2869pegase} & 260 & 6,538 & 2957 & 0 & 259 & 0 \\
   & SDP & \texttt{case9} & 4e-05 & 0.0014 & 5000 & 0 & 0 & 0 \\
   & SDP & \texttt{case14} & 0.0057 & 5.15 & 5000 & 0 & 0 & 0 \\
   & SDP & \texttt{case39} & 4.11 & 351 & 4584 & 0 & 0 & 67 \\
   & SDP & \texttt{case89pegase} & 0.026 & 0.33 & 4750 & 0 & 0 & 28 \\
   & SDP & \texttt{case118} & 2.01 & 4.82 & 4995 & 0 & 0 & 5 \\
   & SDP & \texttt{case300} & 235 & 9,692 & 3894 & 0 & 9 & 39 \\
   & SDP & \texttt{case1354pegase} & 5.96 & 32.9 & 3855 & 0 & 0 & 0 \\
\midrule
  \multirow{2}{*}{Robust Knapsack} & None & \texttt{n=20000} & 0.21 & 0.79 & 28 & 8 & 3789 & 1175 \\
   & None & \texttt{n=80000} & 0.21 & 0.86 & 98 & 16 & 15308 & 4578 \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
  \multirow{1}{*}{QCQP} & Shor &  & 96.1 ms & 94.7 ms \\
\midrule
  \multirow{2}{*}{Inverse Kinematics} & Shor &  & 160 ms & 148 ms \\
   & Lasserre-2 &  & 665 ms & 194 ms \\
\midrule
  \multirow{1}{*}{MIMO Detection} & Shor &  & 92.8 ms & 0.141 ms \\
\midrule
  \multirow{15}{*}{AC-OPF} & SOCP & \texttt{case9} & 2.07 ms & 118 ms \\
   & SOCP & \texttt{case14} & 1.06 ms & 41.8 ms \\
   & SOCP & \texttt{case39} & 2.36 ms & 50.7 ms \\
   & SOCP & \texttt{case89pegase} & 10.3 ms & 100 ms \\
   & SOCP & \texttt{case118} & 7.61 ms & 105 ms \\
   & SOCP & \texttt{case300} & 20.9 ms & 331 ms \\
   & SOCP & \texttt{case1354pegase} & 125 ms & 1.43 s \\
   & SOCP & \texttt{case2869pegase} & -- & -- \\
   & SDP & \texttt{case9} & 23.2 ms & 31.4 ms \\
   & SDP & \texttt{case14} & 40.3 ms & 36.3 ms \\
   & SDP & \texttt{case39} & 107 ms & 55.1 ms \\
   & SDP & \texttt{case89pegase} & 1.34 s & 106 ms \\
   & SDP & \texttt{case118} & 373 ms & 109 ms \\
   & SDP & \texttt{case300} & 1.44 s & 581 ms \\
   & SDP & \texttt{case1354pegase} & -- & -- \\
\midrule
  \multirow{2}{*}{Robust Knapsack} & None & \texttt{n=20000} & -- & 0.156 ms \\
   & None & \texttt{n=80000} & -- & 0.156 ms \\
\bottomrule
\end{tabular}
\end{table}
...
(where calibration samples are re-weighted to account for covariate shift) (CLAUDE to find citation for this).
