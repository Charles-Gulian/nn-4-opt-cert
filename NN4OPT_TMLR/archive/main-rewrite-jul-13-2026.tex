...
\begin{itemize}[itemsep=2pt, parsep=1pt]
    ...
    \item We use convexification techniques to obtain tight lower bounds on the optimal value of non-convex problems, providing training data for our neural networks.
    \item We use split conformal calibration to obtain finite-sample guarantees for certification accuracy.
    \item We demonstrate the power of our framework in several realistic experiments, and explore various speed and accuracy tradeoffs within our framework.
\end{itemize}
...
\end{aligned}
\end{equation}
where $\mathcal{C}(\bm{\theta}) = \{x \in \mathbb{R}^n: \mathbf{c}(\mathbf{x} \,; \bm{\theta}) = \mathbf{0}\}$ for some function $\mathbf{c}(\cdot\,; \bm{\theta})$ parameterized by $\bm{\theta} \in \Theta \subseteq \mathbb{R}^p$. We assume that $f(\cdot\,; \bm{\theta})$ is non-convex or that $\mathbf{c}(\cdot\,;\bm{\theta})$ is nonlinear, making problem \cref{prob:nonconvex}{}{} a non-convex optimization problem. The parameter $\bm{\theta}$ typically represents some time-varying context or input to the optimization problem, such as real and reactive power demand in a power system, or the terrain over which a robotic vehicle moves.

Non-convex problems are an extremely broad class of optimization problem. They are frequently encountered in real-world settings, and yet are notoriously difficult to solve. In fact, because general non-convex problems are NP-hard to solve, most successful solvers rely on bespoke algorithms tailored to specific problems; there are few generalist strategies. Indeed, designing efficient algorithms for specific non-convex problems has remained an extremely active area of research in computer science and operations research for many years.

In this paper, we focus on the \emph{parametric} setting, in which the same non-convex problem \cref{prob:nonconvex}{}{} must be solved repeatedly as the parameter $\bm{\theta}$ varies over $\Theta$. This setting is pervasive in cyber-physical systems, where operators re-solve a fixed problem at high frequency against time-varying data: power system operators dispatch generators every few minutes by solving AC optimal power flow under changing load, communication receivers decode incoming signals by solving MIMO detection problems, and robots plan motions by solving inverse kinematics as targets move. In each case the parameterization is known and stable, but the individual instances must be solved quickly and often under tight real-time budgets.
...
Let $\mathbf{u} \in \mathcal{U}_r$ be a variable in a lifted space, and let $\bm{L}_r: \mathcal{X} \to \mathcal{U}_r$ be a lifting map from the original space to the lifted space, and let $\bm{P}_r: \mathcal{U}_r \to \mathcal{X}$ be a projection (left inverse) from the lifted space to the original space.
...
\begin{aligned}
    v_r(\bm{\theta}) ~=~ \min_{u \in \mathcal{U}_r} \quad & f_r(\mathbf{u}\, ; \bm{\theta)} \\
    \text{s.t.} \quad & \mathbf{u}\in \mathcal{C}_r(\bm{\theta})
\end{aligned}
\end{equation}
where $f_r(\cdot \, ; \bm{\theta})$ is a convex function and $\mathcal{C}_r(\bm{\theta}) \subseteq \mathcal{U}_r$ is a convex set such that
...
For example, multiplying pairs of polynomial constraints of the form 
$g_i(\mathbf{x}) \geq 0$ where $g_i(\mathbf{x}) = \sum_{\bm{\alpha}} g_{i,\bm{\alpha}} \mathbf{x}^{\bm{\alpha}}$ 
(for $i \in \{1, 2, ..., l\}$ and $\bm{\alpha} \in \mathbb{N}^n$, and with $g_{i, \bm{\alpha}} \in \mathbb{R}$ and $\mathbf{x}^{\bm{\alpha}} := x_1^{\alpha_1} x_2^{\alpha_2} \cdots x_n^{\alpha_n}$)
yields new, \textit{valid inequality} constraints:
\begin{align*}
...
Higher order relaxations within the hierarchy linearize the products of larger groups of constraints, yielding progressively tighter formulations with increasing numbers of variables and constraints. 

Other convexification hierarchies use similar lifting and linearization procedures but introduce additional constraints on the lifted variables to yield tighter formulations. For example, the Lasserre (sum-of-squares) hierarchy introduces a lifted \textit{moment matrix} variable $Y$ such that $Y_{\bm{\alpha}, \bm{\beta}}$ replaces the monomial $\mathbf{x}^{\bm{\alpha} + \bm{\beta}}$, along with a positive semi-definite cone constraint $Y \succeq 0$ motivated by the observation that $\mathbf{m}_d(\mathbf{x}) \, \mathbf{m}_d(\mathbf{x})^\top \succeq 0$ for a degree-$d$ monomial basis $\mathbf{m}_d(\mathbf{x}) := [\mathbf{x}^{\bm{\alpha}}: \bm{\alpha} \in \mathbb{N}^n,~ |\bm{\alpha}|\leq d]$ \citep{lasserre2001global}. More recently, \citet{ahmadi2016some} introduced the closely related diagonally dominant sum-of-squares (DSOS/SDSOS) hierarchies, and \citet{rudi2020finding} introduced the kernel sum-of-squares (kernel-SOS) hierarchy.
...
Our framework uses convexification to obtain tight lower bounds on the optimal value of parametric non-convex problems, which are used as training data for a deep neural network. Convexification hierarchies allow us to control the tightness of the lower bounds and the cost of obtaining training data by raising or lowering the order of the relaxation within the hierarchy.

\subsection*{Split Conformal Calibration}



A trained neural network $\hat{v}(\bm{\theta})$ may approximate a relaxation value function $v_r(\bm{\theta})$ well on average, but average-case accuracy is not enough to build a certificate: without a guarantee that holds on individual instances, a single unlucky prediction could silently invalidate the resulting optimality claim. What is needed is a way to convert a point prediction into a bound that is correct with some guaranteed probability, without imposing distributional assumptions on the network's errors or retraining the network itself. Conformal prediction \citep{vovk2005algorithmic} supplies exactly this: given any pre-trained predictor, it constructs prediction sets that provably contain the true value at a user-specified confidence level, using only a finite calibration sample and the assumption that this sample is \textit{exchangeable} with future test instances---no assumption on the model class, the input distribution, or the smoothness of the underlying function is required.

The variant most relevant to our setting is \textit{split} (or \textit{inductive}) conformal prediction \citep{lei2018distribution}, which avoids retraining the model once per calibration point (as classical, ``full'' conformal prediction does) by instead holding out a single calibration set once. Given a predictor $\hat{v}(\cdot)$ and a held-out calibration sample $\{(\bm{\theta}_i, v(\bm{\theta}_i))\}_{i \in \mathcal{I}}$ that the predictor did not see during training, one computes nonconformity scores (typically residuals of the form $v(\bm{\theta}_i) - \hat{v}(\bm{\theta}_i)$) and takes an empirical quantile of these scores at the target miscoverage level $\alpha$. So long as the calibration points and a future test point are exchangeable, the resulting quantile yields a marginal coverage guarantee that holds in finite samples, exactly, with no asymptotic approximation \citep{angelopoulos2023conformal}. Because the guarantee is marginal (averaged over the randomness in the calibration sample and the test point) rather than conditional on a particular $\bm{\theta}$, it is compatible with any regression model, including deep neural networks whose error distributions are otherwise intractable to characterize analytically.

Most applications of conformal prediction construct two-sided intervals for uncertainty quantification. Our use case instead requires a \textit{one-sided} guarantee: to certify optimality we need a value $\hat{v}(\bm{\theta}) - q_\alpha$ that lower-bounds the true relaxation value $v_r(\bm{\theta})$ with probability at least $1 - \alpha$, so that comparing a feasible solution's cost against this calibrated lower bound yields a valid probabilistic certificate. This is a direct application of the one-sided split conformal construction, using only the lower tail of the residual distribution rather than a symmetric two-sided interval; we detail the precise calibration procedure used in our framework in Section~\ref{sec:experiments}. More flexible variants, such as conformalized quantile regression \citep{romano2019conformalized}, calibrate a quantile regression model instead of a fixed residual quantile, which can adapt the width of the guarantee across the parameter space $\Theta$ rather than applying a single offset uniformly; we discuss this direction further in Section~\ref{sec:discussion}.
...
Our proposed approach shifts the computational burden of obtaining near-optimal solutions to an offline learning problem. The power of convexification techniques may thus be distilled into deep neural networks which can assist in online optimization settings.

In convex optimization, global optima may be certified from local information such as the KKT conditions \citep{boyd2004convex}. 
Therefore, one strategy to certify global optima in non-convex problems is to solve a convex relaxation to lower bound $v_r(\bm{\theta}) \leq v(\bm{\theta})$. A feasible solution $\hat{\mathbf{x}} \in \mathcal{C}(\bm{\theta})$ may be compared against $v_r(\bm{\theta})$ to upper bound its \textit{optimality gap}, i.e. the gap between the solution's objective value and the optimal value:
\begin{align*}
    \text{\textit{optimality gap}} ~:=~ f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v(\bm{\theta}) ~\leq~ f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v_r(\bm{\theta}).
\end{align*}

One obvious disadvantage of this approach is that the tightness of the upper bound depends on the size of the \textit{relaxation gap} $:= v(\bm{\theta}) - v_r(\bm{\theta})$, i.e. the gap between the original and relaxed problems' optimal values. It is non-trivial to bound the relaxation gap for an arbitrary convex relaxation of a non-convex problem. However, it is possible to control the relaxation gap by increasing the order of the convex relaxation within some convexification hierarchy (e.g., by introducing new valid inequalities or cutting planes). 
...
\end{equation*}
Unlike solvers that learn a mapping to an optimal solution $\mathbf{x}^*(\bm{\theta})$, the prediction target here is always a scalar, regardless of the dimension of the original decision variable $\mathbf{x}$ or the number of constraints in $\mathcal{C}(\bm{\theta})$. This is one reason the value function tends to be an easier learning target in practice.

\paragraph{3. Calibration} A trained network $\hat{v}(\bm{\theta})$ approximates $v_r(\bm{\theta})$ but offers no guarantee on any individual instance. We close this gap with split conformal calibration: using predictions on a held-out, exchangeable calibration set, we compute the one-sided offset $\tau$ that satisfies
\begin{align*}
    \mathbb{P}(\hat{v}(\bm{\theta}) - v_r(\bm{\theta}) \leq \tau ) \geq 1 - \alpha
\end{align*} at a user-specified miscoverage level $\alpha$. The quantity $\hat{v}(\bm{\theta}) - \tau$ is then a calibrated, probabilistic lower bound on $v_r(\bm{\theta})$ that holds with probability at least $1 - \alpha$.
...
\begin{equation*}
    f(\hat{\mathbf{x}}\, ; \bm{\theta}) ~\leq~ \hat{v}(\bm{\theta}) - \tau + \delta.
\end{equation*}
Because $\hat{v}(\bm{\theta}) - \tau \leq v_r(\bm{\theta}) \leq v(\bm{\theta})$ holds with probability at least $1 - \alpha$, this certificate controls the probability of falsely declaring a solution $\delta$-optimal when its true optimality gap in fact exceeds $\delta$.
Let us analyze the false positive rate (FPR) of this certificate:
\begin{align*}
    \mathit{FPR} ~:=~ \mathbb{P}(f(\hat{\mathbf{x}}\, ; \bm{\theta}) - \hat{v}(\bm{\theta}) \leq \delta - \tau \,|\, f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v(\bm{\theta}) > \delta).
\end{align*}
By adding and subtracting $v(\bm{\theta})$, we may re-write this as
\begin{align*}
    \mathbb{P}(f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v(\bm{\theta}) + v(\bm{\theta}) - \hat{v}(\bm{\theta}) \leq \delta - \tau \,|\, f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v(\bm{\theta}) > \delta)
    \leq \mathbb{P}(\delta + v(\bm{\theta}) - \hat{v}(\bm{\theta}) \leq \delta - \tau),
\end{align*}
which simplifies to $\mathbb{P}(\hat{v}(\bm{\theta}) - v(\bm{\theta}) \geq \tau)$. Since $v_r(\bm{\theta}) \leq v(\bm{\theta})$ for all $\theta \in \Theta$, we have
\begin{align*}
    \mathbb{P}(\hat{v}(\bm{\theta}) - v(\bm{\theta}) \geq \tau) \leq \mathbb{P}(\hat{v}(\bm{\theta}) - v_r(\bm{\theta}) \geq \tau) \leq \alpha
\end{align*}
by construction of $\tau$, hence the FPR of the certificate is bounded above by $\alpha$. In other words, $\tau$ directly controls the FPR. Similarly, the false negative rate (FNR) is controlled by the relaxation gap $v(\bm{\theta}) - v_r(\bm{\theta})$.
\begin{align*}
    \mathit{FNR} ~:=~ \mathbb{P}(f(\hat{\mathbf{x}}\, ; \bm{\theta}) - \hat{v}(\bm{\theta}) > \delta - \tau \,|\, f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v(\bm{\theta}) \leq \delta).
\end{align*}
By adding and subtracting $v(\bm{\theta})$, we may re-write this as
\begin{align*}
    \mathbb{P}(f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v(\bm{\theta}) + v(\bm{\theta}) - \hat{v}(\bm{\theta}) > \delta - \tau \,|\, f(\hat{\mathbf{x}}\, ; \bm{\theta}) - v(\bm{\theta}) \leq \delta)
    \leq \mathbb{P}(\delta + v(\bm{\theta}) - \hat{v}(\bm{\theta}) > \delta - \tau),
\end{align*}
which simplifies to $\mathbb{P}(\hat{v}(\bm{\theta}) - v(\bm{\theta}) < \tau)$. Adding and subtracting $v_r(\bm{\theta})$, we obtain
\begin{align*}
    \mathbb{P}(\hat{v}(\bm{\theta}) - v_r(\bm{\theta}) + v_r(\bm{\theta}) - v(\bm{\theta}) < \tau) = \mathbb{P}(\hat{v}(\bm{\theta}) - v_r(\bm{\theta}) < \tau + v(\bm{\theta}) - v_r(\bm{\theta})).
\end{align*}
This shows that in order to control the FNR, we should reduce both $\tau$ and the relaxation gap $v(\bm{\theta}) - v_r(\bm{\theta})$. However, reducing $\tau$ will necessarily increase the FPR. Therefore, to reduce the FNR without increasing the FPR, the only option is to reduce the relaxation gap, i.e. increase the tightness of the convex relaxation.
...
\begin{figure}[ht!]
    ...
    \begin{minipage}[t]{0.49\textwidth}
        ...
        \caption{IPM convergence behavior for problem \eqref{eq:QCQP_example_prob} for $\bm{\theta} = (-2,1)$. The global minimum is near $(-2.3, -0.5)$, while a spurious local minimum is near $(0.6, 1.8)$.}
    ...
    \begin{minipage}[t]{0.49\textwidth}
        ...
        \caption{Neural network-based value function predictions over target parameter subspace $\Theta = [-5, 5] \times [-5, 5]$ for problem \eqref{eq:QCQP_example_prob}.}
...
The relaxation \cref{prob:qcqp_relaxed} is always exact, i.e. $v_r(\bm{\theta}) = v(\bm{\theta})$ for all $\bm{\theta} \in \mathbb{R}^2$ because problem \cref{prob:qcqp}{}{} has only one quadratic inequality constraint (see $S$-lemma; \cite{boyd2004convex}). Because the relaxation gap is zero everywhere, the error in our experiments is entirely attributable to the neural network's approximation error.

\paragraph{2. Training} We sample $20{,}000$ training instances and $5{,}000$ test instances of $\bm{\theta}$ uniformly at random over $\Theta = [-5,5] \times [-5,5]$. We compute the relaxed optimal value $v_r(\bm{\theta})$ for each instance by solving problem \cref{prob:qcqp_relaxed}{}{}. We then train deep neural networks $\hat{v}(\bm{\theta})$ to approximate $v_r(\bm{\theta})$ via four-fold cross-validation (see Section \ref{sec:experiments} for a detailed description of our neural network training procedure).

\paragraph{3. Calibration} For each fold, we compute the upper $95\%$-quantile of the held-out set prediction errors, $\tau$, which we use to conformalize the neural network's predictions in the deployment phase to control the false positive rate -- i.e., the rate at which our framework erroneously certifies suboptimal solutions as optimal.

\paragraph{4. Deployment}

We solve each problem instance in the test set with IPOPT, a local interior point solver, from a random fixed initialization \citep{wachter2006implementation}. For each feasible local solution $\hat{\mathbf{x}}$, we certify optimality if $f(\hat{\mathbf{x}}\,;\bm{\theta}) \leq \hat{v}(\bm{\theta}) - \tau$. Results are shown in table X.

\begin{table}[H]
\centering
\caption{QCQP example: test-set statistics. ``Relax.\ gap'' is $f(\hat{\mathbf{x}}\,;\bm{\theta}) - v_r(\bm{\theta})$, the gap between the local (IPOPT) solution and the SDP relaxation value.}
\begin{tabular}{ccc}
\toprule
\textbf{Target } $v_r$\textbf{: mean [p5, p95]} & \textbf{Relax.\ gap: mean [p5, p95]} & \textbf{\% infeasible} \\
\midrule
$2.65\ [0.00,\,13.4]$ & $9.76\ [0.00,\,41.3]$ & $0.00$ \\
\bottomrule
\end{tabular}
\label{tab:QCQP-data}
\end{table}
...
\begin{table}[H]
\centering
\caption{QCQP example: neural network prediction error and conformal calibration. Prediction error is mean absolute error (MAE, mean and $5$th--$95$th percentile range over the test set); $q_\alpha$ is the split-conformal offset (mean over the four folds) at each confidence level.}
\begin{tabular}{c ccc}
\toprule
& \multicolumn{3}{c}{\textbf{Conformal offset } $q_\alpha$} \\
\cmidrule(lr){2-4}
\textbf{Abs.\ error: mean [p5, p95]} & $90\%$ & $95\%$ & $99\%$ \\
\midrule
$0.0022\ [0.00,\,0.0094]$ & $0.0038$ & $0.0061$ & $0.0118$ \\
\bottomrule
\end{tabular}
\label{tab:QCQP-error}
\end{table}

\begin{table}[H]
\centering
\caption{QCQP example: certification results at $\delta = 0.1\%$, averaged over the four folds (counts out of $5000$ test instances).}
\begin{tabular}{c cccc}
\toprule
\textbf{Confidence } $1-\alpha$ & \textbf{TP} & \textbf{FP} & \textbf{TN} & \textbf{FN} \\
\midrule
$90\%$ & $593$ & $0$ & $2317$ & $1757$ \\
$95\%$ & $436$ & $0$ & $2317$ & $1914$ \\
$99\%$ & $212$ & $0$ & $2317$ & $2138$ \\
...
\paragraph{Results.} Tables~\ref{tab:QCQP-data}--\ref{tab:QCQP-cert} report the results. Table~\ref{tab:QCQP-data} confirms the SDP relaxation is exact in practice as well as in theory: no test instance was infeasible, and although the mean local-vs-relaxation gap looks large in absolute terms ($9.76$), this reflects the target value's own wide range (up to $13.4$ at the $95$th percentile) rather than a loose relaxation---since $v_r(\bm{\theta}) = v(\bm{\theta})$ exactly here, this gap is entirely attributable to IPOPT's local solutions themselves lying strictly above the true optimum on many instances (i.e., converging to the spurious local minimum illustrated in Figure~\ref{fig:QCQP_example_fig}), not to any relaxation error. Table~\ref{tab:QCQP-error} shows the network fits the value function to high accuracy: mean absolute error is roughly three orders of magnitude smaller than the target's spread, and the conformal offsets increase monotonically with confidence level, as expected. Table~\ref{tab:QCQP-cert} shows zero false positives at every confidence level, so the certificate never mis-certifies a solution on this problem; the true-positive count decreases with confidence level, reflecting the strict $\delta = 0.1\%$ target relative to $\hat{v}(\bm{\theta})$'s residual approximation error. Because the relaxation is exact by construction, this experiment isolates the behavior of the learning and calibration steps of our framework from any confound due to relaxation looseness---a useful complement to the AC-OPF experiments (Section~\ref{sec:experiments}), where relaxation tightness varies substantially and visibly drives certification power.

\section{Experiments} \label{sec:experiments}

We evaluate our framework on four parametric non-convex problems drawn from robotics, digital communications, power systems, and integer programming. Except where noted, every experiment follows the identical five-step protocol described below. We fix a target optimality gap of $\delta = 0.1\%$ throughout and report certification results at the three confidence levels $1 - \alpha \in \{90\%,\, 95\%,\, 99\%\}$.

\subsection*{Experimental Procedure}

\paragraph{(1) Convexification.} For each problem we adopt a convex relaxation whose optimal value $v_r(\bm{\theta})$ lower-bounds the true optimal value, $v_r(\bm{\theta}) \leq v(\bm{\theta})$. For the inverse kinematics, MIMO detection, and AC-OPF problems these are semidefinite or second-order cone relaxations of the underlying QCQP; for inverse kinematics we consider both the first-order (Shor) and second-order Lasserre relaxations. The specific relaxation used in each experiment is described in the corresponding subsection.

\paragraph{(2) Data generation.} We sample $N_{\text{train}} = 20{,}000$ training parameters and $N_{\text{test}} = 5{,}000$ test parameters independently from the problem's parameter distribution. Each training parameter $\bm{\theta}_i$ is labeled with its relaxation value $v_r(\bm{\theta}_i)$. Each test parameter is additionally solved with a fast local or heuristic solver to obtain a feasible solution $\hat{\mathbf{x}}(\bm{\theta}_i)$, whose objective value $f(\hat{\mathbf{x}}\,;\bm{\theta}_i) \geq v(\bm{\theta}_i)$ is a valid upper bound on the true optimum.

\paragraph{(3) Training.} We train a deep neural network $\hat{v}(\bm{\theta})$ to approximate the relaxation value function $v_r(\bm{\theta})$ under mean-squared-error loss, using $4$-fold cross-validation. This yields four fold models per problem; the architecture and optimizer are described in Section~\ref{subsec:dnn}.

\paragraph{(4) Calibration.} We use split conformal calibration to turn each fold model's held-out prediction errors into a probabilistic lower bound on $v_r$. For fold $k$ with validation index set $\mathcal{I}_k$ and target miscoverage $\alpha$, we compute the calibration offset
\begin{equation}
    q^{(k)}_\alpha ~=~ \text{the } \big\lceil (|\mathcal{I}_k| + 1)(1 - \alpha) \big\rceil\text{-th smallest value of } \big\{\, \hat{v}(\bm{\theta}_i) - v_r(\bm{\theta}_i) ~:~ i \in \mathcal{I}_k \,\big\}.
    \label{eq:conformal-offset}
\end{equation}
Because each validation point is exchangeable with a fresh test point, this order statistic guarantees $\Pr\!\big[\, v_r(\bm{\theta}) \geq \hat{v}(\bm{\theta}) - q^{(k)}_\alpha \,\big] \geq 1 - \alpha$, so $\hat{v}(\bm{\theta}) - q^{(k)}_\alpha$ is a valid $(1-\alpha)$-confidence lower bound on the relaxation value, and hence (since $v_r \leq v$) on the true optimum.

\paragraph{(5) Certification.} At test time, given a feasible solution $\hat{\mathbf{x}}$ and target gap $\delta$, we certify $\hat{\mathbf{x}}$ as $\delta$-optimal whenever
\begin{equation}
    f(\hat{\mathbf{x}}\,;\bm{\theta}) ~\leq~ \big(\hat{v}(\bm{\theta}) - q_\alpha\big)(1 + \delta).
    \label{eq:certificate}
\end{equation}
Since $\hat{v} - q_\alpha \leq v_r \leq v$ with probability at least $1 - \alpha$, the certificate \eqref{eq:certificate} controls the probability of falsely certifying a solution whose true relative optimality gap exceeds $\delta$. We evaluate each of the four fold models independently over the full $5{,}000$-point test set, scoring each certificate against the ground-truth event $\big(f(\hat{\mathbf{x}}\,;\bm{\theta}) - v_r(\bm{\theta})\big) / v_r(\bm{\theta}) \leq \delta$ as a true/false positive/negative. We report the resulting false-positive rate (FPR, the rate of erroneous certification) and true-positive rate (TPR, the fraction of genuinely $\delta$-optimal solutions that are certified), each averaged across the four folds.

\subsection*{Deep Neural Networks} \label{subsec:dnn}

Across all experiments we use a single network architecture and training recipe, selected by an ablation study on the AC-OPF problem. The network is a fully-connected feedforward network with six hidden layers of $256$ ReLU units each, mapping the parameter $\bm{\theta}$ to a scalar value-function estimate $\hat{v}(\bm{\theta})$. Network inputs and the regression target are standardized to zero mean and unit variance using statistics computed on the training partition, and predictions are mapped back to physical units at inference.

We train for $1000$ epochs with the AdamW optimizer (learning rate $10^{-3}$, weight decay $10^{-4}$, batch size $256$) under a cosine-annealed learning-rate schedule. We partition the $20{,}000$ training samples with $4$-fold cross-validation (a fixed random shuffle, held constant across all experiments so that the split is reproducible), training one model per fold. Each fold's held-out validation partition serves as the conformal calibration set for that fold model, per step~(4) above. A representative training history is shown in Figure~\ref{fig:training-history}: both the training and validation loss decay by roughly five orders of magnitude and track one another closely throughout training, indicating that the network fits the value function without overfitting.

Figure~\ref{fig:error-boxplot} summarizes the distribution of signed test-set prediction errors $\hat{v}(\bm{\theta}) - v_r(\bm{\theta})$ across all experiments, pooled over the four fold models. The median error is near zero in every experiment, and the spread of the error distribution scales with the magnitude of the problem's optimal value: the power-systems objectives are $O(10^3\text{--}10^5)$, whereas the robotics and communications objectives are $O(1)$.

\begin{figure}[t]
    \centering
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/training_history_qcqp_fold0.png}
    ...
    \begin{minipage}[t]{0.48\textwidth}
        \centering
        \includegraphics[width=\linewidth]{figures/error_distribution_boxplot.png}
        \caption{Distribution of signed test-set prediction errors $\hat{v}(\bm{\theta}) - v_r(\bm{\theta})$ across all experiments (all four folds pooled), on a symmetric-log scale. Medians (red) are near zero throughout; the spread scales with the magnitude of the optimal value.}
        \label{fig:error-boxplot}
    \end{minipage}
\end{figure}

\subsection{Robotics: Inverse Kinematics}

Inverse kinematics is a fundamental problem in robotics: given a target position for a robot's end-effector, one must recover joint configurations that place the end-effector at (or as near as possible to) the target. We consider a two-link planar manipulator with link lengths $\ell_1, \ell_2$ and joint angles $\phi_1, \phi_2$, parameterized by a target position $\bm{\theta} = (x_d, y_d) \in \mathbb{R}^2$. Writing $c_i = \cos\phi_i$ and $s_i = \sin\phi_i$, the end-effector position is $(x_e, y_e) = \big(\ell_1 c_1 + \ell_2 c_{12},~ \ell_1 s_1 + \ell_2 s_{12}\big)$, where $c_{12} = c_1 c_2 - s_1 s_2$ and $s_{12} = s_1 c_2 + c_1 s_2$. The problem of minimizing squared distance to the target is the non-convex QCQP
\begin{equation}
\begin{aligned}
    v(x_d, y_d) ~=~ \min_{c_i, s_i} \quad & (x_e - x_d)^2 + (y_e - y_d)^2 \\
    \text{s.t.} \quad & c_i^2 + s_i^2 = 1, \quad i = 1, 2.
\end{aligned}
\label{eq:IK}
\end{equation}
The trigonometric identities $c_i^2 + s_i^2 = 1$ render the feasible set non-convex.

\paragraph{Experimental setup.} We sample target positions $(x_d, y_d)$ uniformly from a square covering the reachable annulus $\big[\,|\ell_1 - \ell_2|,\, \ell_1 + \ell_2\,\big]$ and slightly beyond, so that both reachable and unreachable targets appear. We label each target with two convex relaxations of \eqref{eq:IK}: the first-order (Shor) semidefinite relaxation and the tighter second-order Lasserre relaxation. Feasible solutions for the test set are obtained by solving \eqref{eq:IK} directly with the interior-point solver IPOPT from a fixed initialization. We train, calibrate, and certify separately for each relaxation, following steps (1)--(5).

\begin{table}[H]
\centering
\caption{Inverse kinematics: test-set statistics. ``Relax.\ gap'' is $f(\hat{\mathbf{x}}\,;\bm{\theta}) - v_r(\bm{\theta})$, the gap between the local solution and the relaxation value (mean and $5$th--$95$th percentile range over the test set). ``Infeasible'' is the percentage of test instances for which either the relaxation or the local solver failed to return a valid value.}
\begin{tabular}{lccc}
\toprule
\textbf{Relaxation} & \textbf{Target } $v_r$\textbf{: mean [p5, p95]} & \textbf{Relax.\ gap: mean [p5, p95]} & \textbf{\% infeasible} \\
\midrule
Shor (order 1)     & $0.036\ [0.00,\,0.25]$ & $0.014\ [0.00,\,0.046]$ & $0.14$ \\
Lasserre (order 2) & $0.039\ [0.00,\,0.25]$ & $0.011\ [0.00,\,0.00]$  & $0.14$ \\
\bottomrule
\end{tabular}
\label{tab:IK-data}
\end{table}
...
\begin{table}[H]
\centering
\caption{Inverse kinematics: neural network prediction error and conformal calibration. Prediction error is mean absolute error (MAE, mean and $5$th--$95$th percentile range over the test set); $q_\alpha$ is the split-conformal offset (mean over the four folds) at each confidence level $1 - \alpha$.}
\begin{tabular}{lcccc}
\toprule
& & \multicolumn{3}{c}{\textbf{Conformal offset } $q_\alpha$} \\
\cmidrule(lr){3-5}
\textbf{Relaxation} & \textbf{Abs.\ error: mean [p5, p95]} & $90\%$ & $95\%$ & $99\%$ \\
\midrule
Shor (order 1)     & $7.7\!\times\!10^{-5}\ [1.3\!\times\!10^{-7},\, 4.1\!\times\!10^{-4}]$ & $1.4\!\times\!10^{-4}$ & $2.7\!\times\!10^{-4}$ & $5.5\!\times\!10^{-4}$ \\
Lasserre (order 2) & $8.4\!\times\!10^{-5}\ [6.3\!\times\!10^{-7},\, 4.1\!\times\!10^{-4}]$ & $1.5\!\times\!10^{-4}$ & $2.6\!\times\!10^{-4}$ & $5.3\!\times\!10^{-4}$ \\
\bottomrule
\end{tabular}
\label{tab:IK-error}
\end{table}
{\footnotesize \noindent Solve times (relaxation / local, seconds per instance): Shor $0.160$ / $0.148$; Lasserre $0.665$ / $0.194$.}

\begin{table}[H]
\centering
\caption{Inverse kinematics: certification results at $\delta = 0.1\%$, averaged over the four folds (counts out of $4993$ test instances).}
\begin{tabular}{lc cccc}
\toprule
\textbf{Relaxation} & \textbf{Confidence } $1-\alpha$ & \textbf{TP} & \textbf{FP} & \textbf{TN} & \textbf{FN} \\
\midrule
\multirow{3}{*}{Shor (order 1)}
 & $90\%$ & $674$ & $0$ & $539$ & $3597$ \\
 & $95\%$ & $418$ & $0$ & $539$ & $3854$ \\
 & $99\%$ & $140$ & $0$ & $539$ & $4131$ \\
\midrule
\multirow{3}{*}{Lasserre (order 2)}
 & $90\%$ & $694$ & $0$ & $223$ & $4076$ \\
 & $95\%$ & $430$ & $0$ & $223$ & $4340$ \\
 & $99\%$ & $146$ & $0$ & $223$ & $4624$ \\
\bottomrule
...
\paragraph{Results.} Tables~\ref{tab:IK-data}--\ref{tab:IK-cert} report the results. Table~\ref{tab:IK-data} shows that the relaxation gap is already small relative to the target scale for both relaxations---consistent with the Shor relaxation being nearly exact for this problem, so the tighter (and roughly four times more expensive) second-order Lasserre relaxation yields no discernible improvement in relaxation tightness. Fewer than $0.15\%$ of test instances were infeasible for either relaxation. Table~\ref{tab:IK-error} shows the network approximates both relaxation value functions to high absolute accuracy: mean absolute error is roughly two to three orders of magnitude smaller than the target's spread, and the calibrated conformal offsets $q_\alpha$ are correspondingly tiny and increase monotonically with confidence level, as expected. Table~\ref{tab:IK-cert} reports certification counts at $\delta = 0.1\%$: the false-positive count is zero at every confidence level for both relaxations, so the certificate never mis-certifies a solution here; the true-positive count decreases with confidence level (a tighter, higher-confidence bound is harder to satisfy), and is comparable between the two relaxations, reflecting their similar tightness. We caution that the optimal value of \eqref{eq:IK} is exactly zero for every reachable target (the end-effector can be placed exactly on the target), so the value function is near zero over much of the parameter space and near-optimality is best assessed in the absolute terms used here rather than relative percentages.

\subsection{Digital Communications: MIMO Detection}

The multiple-input multiple-output (MIMO) detection problem arises in digital communications, where a discrete signal must be recovered from a noisy linear measurement in real time. The problem, its QCQP reformulation, and its SDP relaxation \eqref{eq:MIMO-SDP} are given in Appendix~\ref{sec:experiments-details}; the parameter is the received signal $\bm{\theta} = y \in \mathbb{R}^{2m}$, and each problem instance has $2^n$ feasible points, all of which are local minima.

\paragraph{Experimental setup.} We fix a random $m = 4$ by $n = 2$ complex channel matrix and generate received signals $\bar{y} = \bar{H}x + \epsilon$ from uniformly random transmitted signals $x \in \{-1, 1\}^n$ and Gaussian noise $\epsilon \sim \mathcal{N}(0, \sigma^2 I)$ with $\sigma^2 = 0.1$ (Appendix~\ref{sec:experiments-details}). We label each received signal with the SDP relaxation value \eqref{eq:MIMO-SDP}, and obtain feasible solutions for the test set with a fast zero-forcing detector. We then train, calibrate, and certify per steps (1)--(5).

\begin{table}[H]
\centering
\caption{MIMO detection: test-set statistics. ``Relax.\ gap'' is $f(\hat{\mathbf{x}}\,;\bm{\theta}) - v_r(\bm{\theta})$, the gap between the zero-forcing detector's solution and the SDP relaxation value.}
\begin{tabular}{ccc}
\toprule
\textbf{Target } $v_r$\textbf{: mean [p5, p95]} & \textbf{Relax.\ gap: mean [p5, p95]} & \textbf{\% infeasible} \\
\midrule
$0.79\ [0.27,\,1.58]$ & $0.0023\ [0.00,\,0.00]$ & $0.00$ \\
\bottomrule
\end{tabular}
\label{tab:MIMO-data}
\end{table}
...
\begin{table}[H]
\centering
\caption{MIMO detection: neural network prediction error and conformal calibration. Prediction error is mean absolute error (MAE, mean and $5$th--$95$th percentile range over the test set); $q_\alpha$ is the split-conformal offset (mean over the four folds) at each confidence level.}
\begin{tabular}{c ccc}
\toprule
& \multicolumn{3}{c}{\textbf{Conformal offset } $q_\alpha$} \\
\cmidrule(lr){2-4}
\textbf{Abs.\ error: mean [p5, p95]} & $90\%$ & $95\%$ & $99\%$ \\
\midrule
$0.017\ [0.001,\,0.047]$ & $0.023$ & $0.033$ & $0.057$ \\
\bottomrule
\end{tabular}
\label{tab:MIMO-error}
\end{table}

\begin{table}[H]
\centering
\caption{MIMO detection: certification results at $\delta = 0.1\%$, averaged over the four folds (counts out of $5000$ test instances).}
\begin{tabular}{c cccc}
\toprule
\textbf{Confidence } $1-\alpha$ & \textbf{TP} & \textbf{FP} & \textbf{TN} & \textbf{FN} \\
\midrule
...
\paragraph{Results.} Tables~\ref{tab:MIMO-data}--\ref{tab:MIMO-cert} report the results. Table~\ref{tab:MIMO-data} shows the relaxation gap is small relative to the target scale (mean $0.0023$ against mean target $0.79$), indicating the SDP relaxation is tight for this problem, and no test instances were infeasible. Table~\ref{tab:MIMO-error} shows a mean absolute prediction error of $0.017$, roughly $2\%$ of the mean target value---a good but noticeably less tight fit than in the other experiments, consistent with the received signal $\bm{\theta} = y$ having a lower signal-to-noise ratio than the smoother parameterizations of the other problems. The conformal offsets increase monotonically with confidence level, as expected. Table~\ref{tab:MIMO-cert} shows the false-positive count is small at every confidence level ($2.8$, $1.2$, and $0.2$ out of roughly $63$--$65$ truly sub-optimal detections)---that is, the certificate rarely declares a sub-optimal detection to be $\delta$-optimal. The true-positive count is more modest, reflecting the strict $\delta = 0.1\%$ target: because the conformal offset $q_\alpha$ is comparable in size to $\delta$, the certificate \eqref{eq:certificate} is conservative and abstains on many genuinely optimal detections. Relaxing $\delta$ or improving prediction accuracy would raise the certification rate. In terms of speed, the SDP relaxation requires $\sim\!93$~ms per instance, whereas a single network prediction takes $\sim\!1~\mu$s (a batch of $5{,}000$ predictions completes in under $6$~ms), a speedup of roughly five orders of magnitude.

\subsection{Power Systems Optimization: AC Optimal Power Flow}

The AC optimal power flow (AC-OPF) problem is solved by grid operators every few minutes to schedule generation at minimum cost subject to the physics of the network. It is a non-convex QCQP whose full formulation, parameterized by the real and reactive load $\bm{\theta} = (P_D, Q_D)$ at each bus, is given in Appendix~\ref{sec:experiments-details}. We study eight standard IEEE and PEGASE test systems ranging from $9$ to $2{,}869$ buses.

\paragraph{Experimental setup.} For each test system we sample load profiles and label them with two convex relaxations of AC-OPF: the second-order cone (SOCP) relaxation and the (tighter) chordal semidefinite relaxation. Feasible solutions for the test set are obtained with the interior-point solver IPOPT. Both relaxations lower-bound the true optimum, but differ substantially in tightness and cost: the SOCP relaxation is cheaper to solve, while the chordal SDP is tighter. We train, calibrate, and certify separately for each relaxation and each test system, following steps (1)--(5). The chordal SDP was intractable at our time budget for the largest system (\texttt{case2869pegase}), which we report for the SOCP relaxation only.

\begin{table}[H]
\centering
\footnotesize
\caption{AC-OPF test-set statistics, SOCP relaxation. ``Relax.\ gap'' is $f(\hat{\mathbf{x}}\,;\bm{\theta}) - v_r(\bm{\theta})$ (mean and $5$th--$95$th percentile range over the test set); ``\% infeasible'' is the share of test instances for which either the relaxation or local solver failed. A negative gap (\texttt{case89pegase}) indicates the local solution fell below the SOCP value here, i.e.\ the relaxation was not a valid lower bound for this system; we exclude its certification results below and flag it for further investigation.}
\begin{tabular}{lcccc}
\toprule
\textbf{Test system} & \textbf{\# Buses} & \textbf{Target } $v_r$\textbf{: mean [p5, p95]} & \textbf{Relax.\ gap: mean [p5, p95]} & \textbf{\% infeas.} \\
\midrule
\texttt{case9}          & $9$    & $4692\ [2757,\,7336]$          & $0.040\ [0.00,\,0.15]$    & $0.0$ \\
\texttt{case14}         & $14$   & $7049\ [4300,\,10230]$         & $5.81\ [3.02,\,7.52]$     & $0.0$ \\
\texttt{case39}         & $39$   & $33520\ [16370,\,55580]$       & $10.7\ [2.04,\,18.4]$     & $7.0$ \\
\texttt{case89pegase}   & $89$   & $7338\ [5182,\,9536]$          & $-71.3\ [-73.5,\,-69.8]$\textsuperscript{$\dagger$} & $4.9$ \\
\texttt{case118}        & $118$  & $113200\ [70640,\,158900]$     & $274\ [140,\,368]$        & $0.0$ \\
\texttt{case300}        & $300$  & $586100\ [395600,\,810300]$    & $1091\ [634,\,1462]$      & $19.7$ \\
\texttt{case1354pegase} & $1354$ & $67330\ [46940,\,88080]$       & $102\ [82,\,130]$         & $0.3$ \\
\texttt{case2869pegase} & $2869$ & $126700\ [86600,\,163600]$     & $132\ [-62,\,1480]$       & $35.8$ \\
\bottomrule
\end{tabular}
\label{tab:ACOPF-data-socp}
\end{table}

\begin{table}[H]
\centering
\footnotesize
\caption{AC-OPF test-set statistics, chordal SDP relaxation. Same conventions as Table~\ref{tab:ACOPF-data-socp}. \texttt{case2869pegase} is omitted: the chordal SDP was intractable at our time budget for this system.}
\begin{tabular}{lcccc}
\toprule
\textbf{Test system} & \textbf{\# Buses} & \textbf{Target } $v_r$\textbf{: mean [p5, p95]} & \textbf{Relax.\ gap: mean [p5, p95]} & \textbf{\% infeas.} \\
\midrule
\texttt{case9}          & $9$    & $4692\ [2757,\,7336]$        & $4.0\!\times\!10^{-5}\ [3\!\times\!10^{-6},\,1.3\!\times\!10^{-4}]$ & $0.0$ \\
\texttt{case14}         & $14$   & $7054\ [4303,\,10240]$       & $0.0057\ [-0.001,\,0.011]$ & $0.0$ \\
\texttt{case39}         & $39$   & $33520\ [16370,\,55580]$     & $4.11\ [0.00,\,11.5]$      & $7.0$ \\
\texttt{case89pegase}   & $89$   & $7281\ [5116,\,9495]$        & $0.026\ [-0.017,\,0.153]$  & $4.4$ \\
\texttt{case118}        & $118$  & $113500\ [70790,\,159300]$   & $2.02\ [1.43,\,2.74]$      & $0.0$ \\
\texttt{case300}        & $300$  & $583500\ [395800,\,804100]$  & $235\ [34,\,417]$          & $21.2$ \\
\texttt{case1354pegase} & $1354$ & $67680\ [47100,\,88090]$     & $5.96\ [1.86,\,9.47]$      & $22.9$ \\
\bottomrule
\end{tabular}
\label{tab:ACOPF-data-chordal}
\end{table}

\begin{table}[H]
\centering
\footnotesize
\caption{AC-OPF neural network prediction error and conformal calibration, SOCP relaxation. Absolute error (mean, $5$th--$95$th percentile range) and conformal offsets $q_\alpha$ (mean over the four folds) are in the same units as the relaxation value $v_r$ (Table~\ref{tab:ACOPF-data-socp}).}
\begin{tabular}{lc ccc}
\toprule
& & \multicolumn{3}{c}{\textbf{Conformal offset } $q_\alpha$} \\
\cmidrule(lr){3-5}
\textbf{Test system} & \textbf{Abs.\ error: mean [p5, p95]} & $90\%$ & $95\%$ & $99\%$ \\
\midrule
\texttt{case9}          & $0.72\ [0.04,\,2.13]$    & $1.13$  & $1.63$  & $2.79$  \\
\texttt{case14}         & $2.82\ [0.07,\,12.2]$    & $4.66$  & $8.64$  & $17.5$  \\
\texttt{case39}         & $33.2\ [0.57,\,142]$     & $53.3$  & $100$   & $219$   \\
\texttt{case89pegase}   & $2.50\ [0.03,\,13.1]$    & $3.40$  & $8.07$  & $19.8$  \\
\texttt{case118}        & $27.8\ [0.74,\,144]$     & $21.4$  & $63.7$  & $270$   \\
\texttt{case300}        & $267\ [8.2,\,1179]$      & $478$   & $999$   & $2325$  \\
\texttt{case1354pegase} & $4.11\ [0.18,\,13.2]$    & $6.77$  & $10.1$  & $17.8$  \\
\texttt{case2869pegase} & $7.64\ [0.34,\,24.1]$    & $12.7$  & $18.8$  & $31.9$  \\
\bottomrule
\end{tabular}
\label{tab:ACOPF-error-socp}
\end{table}

\begin{table}[H]
\centering
\footnotesize
\caption{AC-OPF neural network prediction error and conformal calibration, chordal SDP relaxation. Same conventions as Table~\ref{tab:ACOPF-error-socp}.}
\begin{tabular}{lc ccc}
\toprule
& & \multicolumn{3}{c}{\textbf{Conformal offset } $q_\alpha$} \\
\cmidrule(lr){3-5}
\textbf{Test system} & \textbf{Abs.\ error: mean [p5, p95]} & $90\%$ & $95\%$ & $99\%$ \\
\midrule
\texttt{case9}          & $0.73\ [0.05,\,2.14]$    & $1.10$  & $1.60$  & $2.71$  \\
\texttt{case14}         & $2.94\ [0.07,\,12.3]$    & $4.93$  & $8.93$  & $18.3$  \\
\texttt{case39}         & $30.8\ [0.63,\,136]$     & $47.5$  & $88.3$  & $196$   \\
\texttt{case89pegase}   & $2.52\ [0.03,\,13.0]$    & $3.55$  & $8.00$  & $19.1$  \\
\texttt{case118}        & $28.4\ [0.79,\,139]$     & $22.9$  & $67.4$  & $281$   \\
\texttt{case300}        & $237\ [8.5,\,951]$       & $356$   & $607$   & $1485$  \\
\texttt{case1354pegase} & $4.84\ [0.23,\,15.0]$    & $7.52$  & $10.9$  & $19.4$  \\
\bottomrule
\end{tabular}
\label{tab:ACOPF-error-chordal}
\end{table}

\begin{table}[H]
\centering
\footnotesize
\caption{AC-OPF certification results at $\delta = 0.1\%$, SOCP relaxation, averaged over the four folds (counts out of $5000$ test instances, fewer where infeasible instances were dropped; see Table~\ref{tab:ACOPF-data-socp}). \texttt{case89pegase} is excluded (invalid relaxation lower bound; see Table~\ref{tab:ACOPF-data-socp}).}
\begin{tabular}{lc cccc}
\toprule
\textbf{Test system} & \textbf{Confidence } $1-\alpha$ & \textbf{TP} & \textbf{FP} & \textbf{TN} & \textbf{FN} \\
\midrule
\multirow{3}{*}{\texttt{case9}} & $90\%$ & $4971$ & $0$ & $0$ & $29$ \\
 & $95\%$ & $4947$ & $0$ & $0$ & $53$ \\
 & $99\%$ & $4463$ & $0$ & $0$ & $538$ \\
\midrule
\multirow{3}{*}{\texttt{case14}} & $90\%$ & $589$ & $66$ & $475$ & $3870$ \\
 & $95\%$ & $243$ & $25$ & $516$ & $4216$ \\
 & $99\%$ & $47$  & $4$  & $537$ & $4412$ \\
\midrule
\multirow{3}{*}{\texttt{case39}} & $90\%$ & $966$ & $0$ & $124$ & $3560$ \\
 & $95\%$ & $353$ & $0$ & $124$ & $4174$ \\
 & $99\%$ & $61$  & $0$ & $124$ & $4466$ \\
\midrule
\multirow{3}{*}{\texttt{case118}} & $90\%$ & $0$ & $74$  & $4926$ & $0$ \\
 & $95\%$ & $0$ & $58$  & $4942$ & $0$ \\
 & $99\%$ & $0$ & $12$  & $4988$ & $0$ \\
\midrule
\multirow{3}{*}{\texttt{case300}} & $90\%$ & $0$ & $160$ & $3856$ & $0$ \\
 & $95\%$ & $0$ & $79$  & $3936$ & $0$ \\
 & $99\%$ & $0$ & $12$  & $4003$ & $0$ \\
\midrule
\multirow{3}{*}{\texttt{case1354pegase}} & $90\%$ & $0$ & $8$ & $4978$ & $0$ \\
 & $95\%$ & $0$ & $6$ & $4980$ & $0$ \\
 & $99\%$ & $0$ & $3$ & $4983$ & $0$ \\
\midrule
\multirow{3}{*}{\texttt{case2869pegase}} & $90\%$ & $2949$ & $0$ & $260$ & $0$ \\
 & $95\%$ & $2949$ & $0$ & $260$ & $0$ \\
 & $99\%$ & $2948$ & $0$ & $260$ & $1$ \\
\bottomrule
\end{tabular}
\label{tab:ACOPF-cert-socp}
\end{table}

\begin{table}[H]
\centering
\footnotesize
\caption{AC-OPF certification results at $\delta = 0.1\%$, chordal SDP relaxation. Same conventions as Table~\ref{tab:ACOPF-cert-socp}.}
\begin{tabular}{lc cccc}
\toprule
\textbf{Test system} & \textbf{Confidence } $1-\alpha$ & \textbf{TP} & \textbf{FP} & \textbf{TN} & \textbf{FN} \\
\midrule
\multirow{3}{*}{\texttt{case9}} & $90\%$ & $4972$ & $0$ & $0$ & $28$ \\
 & $95\%$ & $4943$ & $0$ & $0$ & $57$ \\
 & $99\%$ & $4580$ & $0$ & $0$ & $420$ \\
\midrule
\multirow{3}{*}{\texttt{case14}} & $90\%$ & $3366$ & $0$ & $1$ & $1632$ \\
 & $95\%$ & $1734$ & $0$ & $1$ & $3265$ \\
 & $99\%$ & $155$  & $0$ & $1$ & $4844$ \\
\midrule
\multirow{3}{*}{\texttt{case39}} & $90\%$ & $1373$ & $0$ & $72$ & $3206$ \\
 & $95\%$ & $440$  & $0$ & $72$ & $4139$ \\
 & $99\%$ & $72$   & $0$ & $72$ & $4507$ \\
\midrule
\multirow{3}{*}{\texttt{case89pegase}} & $90\%$ & $4332$ & $0$ & $0$ & $446$ \\
 & $95\%$ & $2033$ & $0$ & $0$ & $2745$ \\
 & $99\%$ & $147$  & $0$ & $0$ & $4631$ \\
\midrule
\multirow{3}{*}{\texttt{case118}} & $90\%$ & $4795$ & $0$ & $0$ & $205$ \\
 & $95\%$ & $4391$ & $0$ & $0$ & $609$ \\
 & $99\%$ & $100$  & $0$ & $0$ & $4900$ \\
\midrule
\multirow{3}{*}{\texttt{case300}} & $90\%$ & $1656$ & $0$ & $75$ & $2211$ \\
 & $95\%$ & $940$  & $0$ & $75$ & $2928$ \\
 & $99\%$ & $112$  & $0$ & $75$ & $3755$ \\
\midrule
\multirow{3}{*}{\texttt{case1354pegase}} & $90\%$ & $3848$ & $0$ & $0$ & $8$ \\
 & $95\%$ & $3846$ & $0$ & $0$ & $9$ \\
 & $99\%$ & $3843$ & $0$ & $0$ & $12$ \\
\bottomrule
\end{tabular}
\label{tab:ACOPF-cert-chordal}
\end{table}

\paragraph{Results.} Tables~\ref{tab:ACOPF-data-socp}--\ref{tab:ACOPF-cert-chordal} report the results. Tables~\ref{tab:ACOPF-data-socp}--\ref{tab:ACOPF-data-chordal} show that the chordal SDP relaxation is substantially tighter than the SOCP relaxation on every test system---for example, on \texttt{case118} the mean relaxation gap falls from $274$ (SOCP) to $2.0$ (chordal), and on \texttt{case1354pegase} from $102$ to $6.0$---at the cost of being intractable at our time budget for the largest system (\texttt{case2869pegase}). The infeasibility rate (instances where either the relaxation or local solver failed) ranges from $0\%$ on the small systems to as high as $36\%$ on \texttt{case2869pegase}, reflecting the increasing numerical difficulty of both problems at scale. On \texttt{case89pegase}, the SOCP relaxation value exceeded the local solution on average (mean gap $-71.3$), indicating the relaxation failed to lower-bound the true optimum for this configuration; we exclude it from certification and flag it for further investigation.

Tables~\ref{tab:ACOPF-error-socp}--\ref{tab:ACOPF-error-chordal} show the network approximates both relaxation value functions to high accuracy relative to their scale: mean absolute error is consistently $1$--$2$ orders of magnitude smaller than the mean target value across all eight systems and both relaxations, and this accuracy is remarkably stable across three orders of magnitude in system size. The conformal offsets $q_\alpha$ increase monotonically with confidence level in every configuration, as expected, and scale with the network's prediction error.

The certification behavior in Tables~\ref{tab:ACOPF-cert-socp}--\ref{tab:ACOPF-cert-chordal} is governed by the tightness of the relaxation. Under the chordal SDP, which is tight for every system, true positives dominate at every confidence level, e.g.\ $3846$ out of $3855$ instances certified on \texttt{case1354pegase} at $95\%$ confidence, with zero false positives throughout. Under the looser SOCP relaxation, the picture is starker: on the larger systems (\texttt{case118}, \texttt{case300}, \texttt{case1354pegase}), the relaxation gap itself exceeds $\delta = 0.1\%$ for essentially every instance, so there are zero true positives---no solution is $\delta$-optimal \emph{with respect to the SOCP bound}---and the small nonzero false-positive counts (e.g.\ $74$ out of $5000$ on \texttt{case118} at $90\%$ confidence) are exactly the instances the conformal guarantee allows to slip through at rate $1-\alpha$. This illustrates a central property of our framework: the quality of the certificate is inherited from the quality of the underlying relaxation, and a tighter relaxation (higher in a convexification hierarchy) directly enables certification at a stricter optimality tolerance.

Finally, the speed advantage is decisive: a single network prediction costs $\sim\!1~\mu$s, whereas solving the convex relaxations offline required seconds to minutes per instance---the largest systems needed a per-solve wall-clock limit of up to $600$~s during data generation. The network thus compresses a relaxation that is far too slow for real-time use into a millisecond-scale certificate.

\subsection{Integer Programming: Robust 0-1 Knapsack}

This experiment is a little different. We use larger data set, and we are interested in actually speeding up branch-and-bound.

\subsubsection*{Accelerating Branch-and-Bound with Deep Neural Networks}
...
Discuss validity of exchangeability assumption; other potential shortfalls; but also strengths of this approach and its ability to complement learned solution mappings and machine learning-based solvers. Also discuss potential to use CQR within this framework, and some preliminary results on this front if there is time.
