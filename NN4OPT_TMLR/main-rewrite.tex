...
We fix a random channel matrix with $m = 4$, $n = 2$, sample transmitted signals $x \in \{-1, +1\}^n$ uniformly with noise variance $\sigma^2 = 0.1$, and obtain feasible solutions with a fast zero-forcing detector (a linear pseudo-inverse of $H$ followed by rounding). Because $v(\bm{\theta})$ is a minimum over the $2^n$ candidate signals, it is continuous but has kinks at the maximum-likelihood decision boundaries where the arg-min switches, making it a comparatively rough target relative to our other experiments, and the network's prediction error is correspondingly larger relative to the small spread of the optimal value. Despite this, our fixed channel is well-conditioned and over-determined ($m=4$ measurements for $n=2$ unknowns; condition number $\approx 1.9$), so the zero-forcing detector recovers the exact maximum-likelihood solution on over $99.9\%$ of test instances, and the network's prediction error remains small enough that the certificate fires on nearly all of them (Table~\ref{tab:combined}).
...
\midrule
  \multirow{1}{*}{MIMO Detection} & Shor &  & 0.79 & 0.79 & 0.8 & 0.002 & 0.0071 & 0.012 & 4811 & 0 & 4 & 185 & 92.8 ms & 0.14 ms \\
\midrule
...
\midrule
  \multirow{2}{*}{Robust Knapsack} & Exact & \texttt{n=25} & 281 & 281 & 272 & 1.23 & 3.96 & 1.41 & 50 & 4 & 4238 & 708 & 41.0 ms & 0.0687 ms \\
   & SOCP & \texttt{n=25} & 283 & 281 & 272 & 1.16 & 3.66 & 1.41 & 7 & 1 & 4241 & 751 & 0.68 ms & 0.0687 ms \\
\bottomrule
\end{tabular}%
}
\end{sidewaystable}
...
For robust knapsack, whose parameter enters the feasible set rather than the objective, the optimal value has genuine argmax-switching discontinuities in $\bm\theta$, and we find the resulting certification-accuracy floor to be irreducible: it does not improve with $4\times$ more training data or $5\times$ more network capacity (Appendix~\ref{app:ablation}), illustrating a ``preservation of difficulty'' for this class of combinatorial problems that no amount of offline compute closes.
...
\subsection{Robust Knapsack: Data-Size and Capacity Ablation}
...
ablate whether more training data or a larger network closes the resulting certification-accuracy
gap. Table~\ref{tab:knapsack-ablation} reports $\hat q_{99}/\mathrm{std}(v^\ast)$ -- the calibrated
conformal offset as a fraction of the optimal value's spread, the quantity that determines how
tight a certification threshold $\delta$ can be -- for the exact-MISOCP target ($6\times256$
network, our default architecture throughout the paper) at $20$k and $80$k training instances, and
for two larger architectures at $20$k instances. Neither a $4\times$ increase in training data nor
a $5\times$ increase in parameter count meaningfully reduces $\hat q_{99}/\mathrm{std}(v^\ast)$,
consistent with an irreducible floor set by the discontinuities themselves rather than by
insufficient data or network capacity.

\begin{table}[h]
\centering\footnotesize
\caption{Robust knapsack (correlated-$\bm\mu$, exact-MISOCP target) data-size and capacity
ablation: $4$-fold cross-validated $\hat q_{99}/\mathrm{std}(v^\ast)$. Neither more data
(first two rows) nor more capacity (last two rows, at $20$k instances) closes the gap.}
\label{tab:knapsack-ablation}
\begin{tabular}{c c c}
\toprule
\textbf{Training instances} & \textbf{Architecture} & $\hat q_{99}/\mathrm{std}(v^\ast)$ \\
\midrule
  $20$k & $6\times256$ (default) & $0.0845$ \\
  $80$k & $6\times256$ & $0.0848$ \\
\midrule
  $20$k & $6\times512$ & $0.0818$ \\
  $20$k & $8\times512$ & $0.0831$ \\
\bottomrule
\end{tabular}
\end{table}
