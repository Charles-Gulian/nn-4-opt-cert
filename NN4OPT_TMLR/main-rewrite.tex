...
We demonstrate the power of the proposed framework on several realistic problems from power systems, robotics, signal processing, and integer optimization. Our results indicate that neural networks can effectively identify suboptimal solutions in real-time, potentially leading to improved solution accuracy without compromising speed.
\end{abstract}
...
Each experiment instantiates the framework of Section~\ref{sec:approach} for a particular non-convex optimization problem.
We fix a convex relaxation of the problem and generate training data by randomly sampling
...
We then train a four-fold ensemble of deep neural networks $\hat{v}(\bm{\theta})$ to approximate the relaxed value function $v_r(\bm{\theta})$ and calibrate each fold on its own held-out partition. Every test parameter is additionally solved by a fast local or heuristic solver to obtain the feasible solution $\hat{\mathbf{x}}$ that the framework then certifies using the trained and calibrated neural network predictions. We vary these ingredients across experiments, comparing convex relaxations of different orders (first- vs.\ second-order Lasserre SDP relaxation for inverse kinematics, SOCP vs.\ chordal SDP relaxation for AC-OPF), different fast solvers, different training data sizes, and studying problem scales spanning three orders of magnitude.

We deploy the certificate of Section~\ref{sec:approach}, certifying $\hat{\mathbf{x}}$ as $\delta$-optimal when $f(\hat{\mathbf{x}}\,;\bm{\theta}) \leq \hat{v}(\bm{\theta}) - \hat{q}_\alpha + \delta$, at $\alpha = 0.01$ (conformal offset $\hat q_{99}$) throughout the experiments, with the optimality tolerance $\delta$ set per experiment. This keeps $\delta$ meaningful across problems of vastly different magnitude and avoids the instability associated with relative optimality gaps when the optimal value is near zero. In each experiment, $\delta$ is chosen to reflect a notion of near-optimality for the optimization problem at the scale of the prediction error of the trained neural network. We score each certificate against the ground-truth event $f(\hat{\mathbf{x}}\,;\bm{\theta}) - v(\bm{\theta}) \leq \delta$. The exact optimal value $v(\bm\theta)$ is available for QCQP, inverse kinematics, small-scale MIMO detection, and robust knapsack. For large-scale AC-OPF and MIMO detection problems, no tractable exact solver exists, and we substitute the tightest relaxation value available for the ground truth optimal value. Since $v_r(\bm{\theta})$ lower-bounds the true optimal value, it makes the ground-truth event strictly harder to satisfy and so yields conservative rather than optimistic false-positive counts. We report true/false positive/negative counts (TP, FP, TN, FN) averaged over the four folds to demonstrate the certification accuracy of our framework in each problem setting for the chosen optimality tolerance $\delta$. Results are summarized in Table~\ref{tab:combined} and discussed in detail in the following subsections.

\subsection*{Deep Neural Networks} \label{subsec:dnn}

Across all experiments we use a single network architecture and training recipe, selected by an ablation study on the AC-OPF problem (Appendix~\ref{app:ablation}). The network is a fully-connected feedforward network with four hidden layers of $64$ ReLU units each, mapping the parameter $\bm{\theta}$ to a scalar value function estimate $\hat{v}(\bm{\theta})$. Network inputs and the regression target are standardized to zero mean and unit variance using statistics computed on the training partition, and predictions are mapped back to physical units at inference.

We train for $1500$ epochs with the AdamW optimizer (learning rate $10^{-3}$, weight decay $10^{-4}$, batch size $256$) under a cosine-annealed learning-rate schedule. We partition the $20{,}000$ training samples with $4$-fold cross-validation (a fixed random shuffle, held constant across all experiments so that the split is reproducible), training one model per fold. Each fold's heldout validation partition serves as the conformal calibration set for that fold model, per step~(4) above. A representative training history is shown in Figure~\ref{fig:training-history}: both the training and validation loss decay by roughly five orders of magnitude and track one another closely throughout training, indicating that the network fits the value function without overfitting.
...
We convexify problem \cref{prob:IK}{}{} by semidefinite lifting of the monomials in $\mathbf{y} = (c_1, s_1, c_2, s_2)$. The first-order Lasserre (Shor) relaxation introduces a moment matrix $Y \succeq 0$ with $Y_{00} = 1$ and linearizes the objective and the unit-circle constraints $c_i^2 + s_i^2 = 1$ in the entries of $Y$. The second-order Lasserre relaxation adds the degree-two moments for a tighter but larger SDP. 

For our experiment, we choose $\ell_1 = 1$ and $\ell_2 = 0.5$. We sample target positions $\bm{\theta}$ uniformly over the square $[0, 1.65]^2$ covering the reachable annulus $[0.5, 1.5]$ and slightly beyond, so that both reachable and unreachable targets appear. We obtain feasible solutions by solving problem \cref{prob:IK}{}{} with an interior point method (IPOPT). The ground-truth value function may be computed analytically in this experiment, which lets us exactly test the certification accuracy of our framework. Figure~\ref{fig:ik-value-function} visualizes this value function alongside the two learned approximations.

This experiment demonstrates an important aspect of our framework: the change from the first-order to second-order Lasserre hierarchy relaxation yields a tighter (exact) formulation that is more expensive to solve. This is a central tradeoff in our framework: tighter convex relaxations provide more accurate optimality certificates, but are more expensive to solve for generating training data. On the other hand, insufficient training data can worsen approximation error, leading to less accurate optimality certificates. In this small-scale experiment, the cost of obtaining training data for the tighter second-order Lasserre relaxation is negligible though still larger than the first-order relaxation, and the resulting improvement in certificate accuracy is significant. In particular, the looser first-order relaxation yields higher false negative certifications in the region inside of the reachable annulus. In larger-scale problems, the cost of obtaining training data for tighter relaxations may be prohibitive, and our framework allows practitioners to choose a relaxation that balances certificate accuracy with training data generation cost.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_ik_value_function_comparison.pdf}
    \caption{The true value function $v(\bm{\theta})$ for the inverse kinematics problem (left) and the neural network's learned approximation $\hat{v}(\bm{\theta})$ trained on the first-order Lasserre (Shor) relaxation (center) and the tighter second-order Lasserre relaxation (right), over target positions $\bm{\theta} = (\theta_1, \theta_2)$. Notably, the first-order Lasserre relaxation is not exact, and the neural network trained on it underestimates the true value function in the region inside the reachable annulus. The second-order Lasserre relaxation is exact, and the neural network trained on it closely matches the true value function.}
...
We consider two problem scales in this experiment. The first experiment fixes a random channel matrix with $m = 4$, $n = 2$, samples transmitted signals $x \in \{-1, +1\}^n$ uniformly with noise variance $\sigma^2 = 0.2$, and obtains feasible solutions with a fast zero-forcing detector (a linear pseudo-inverse of $H$ followed by rounding). The feasible set is a discrete set of $2^n$ candidate signals, and the optimal value function $v(\bm{\theta})$ is piecewise quadratic in $\bm{\theta}$, with a different quadratic for each candidate signal. The optimal value function is therefore highly non-smooth, with kinks at the boundaries between the $2^n$ candidate signals' Voronoi cells in $\mathbb{R}^{2m}$, and the network's prediction error is correspondingly larger relative to the small spread of the optimal value. Despite this, our fixed channel is well-conditioned and over-determined ($m=4$ measurements for $n=2$ unknowns; condition number $= 1.9$), so the zero-forcing detector recovers the exact maximum-likelihood solution on $94.4\%$ of test instances, and the network's prediction error remains small enough so that we certify nearly all of them (Table~\ref{tab:combined}).

The second scale ($m=32$, $n=16$) has a much higher-dimensional input ($2m=64$ versus $8$ for the small case). The channel matrix is again a single random draw with noise level $\sigma^2=5$, corresponding to a lower signal-to-noise ratio. The zero-forcing detector recovers the exact maximum-likelihood solution on only $89.9\%$ of test instances, and the network's prediction error is larger relative to the spread of the optimal value function, so we certify fewer of them (Table~\ref{tab:combined}).
The network's relative error grows substantially (MAE $= 9.6\%$ of the mean cost, versus $2.6\%$ at the small scale). For comparison, a single bit flip of the recovered signal typically changes the objective value by roughly $244$ (median; $5$th-percentile $106$), so a certified solution is usually, but not always, truly optimal at our chosen tolerance of $\delta = 160$. The certified fraction is lower than at the small scale (TPR is $94\%$ versus $99.7\%$), reflecting the network's own accuracy at this input dimension.
...
Let $Y = G + j B$ be the bus admittance matrix of the network.
...
Each demand sample is generated by perturbing the network's nominal demand using uniform scaling factors: a single system-wide scaling factor $\alpha \sim \mathrm{Uniform}(0.6, 1.2)$ is drawn per sample, together with independent per-bus multipliers $\eta_i \sim \mathrm{Uniform}(0.7, 1.3)$, so that $p_{Di} = \alpha\,\eta_i\,p_{Di}^{0}$ and $q_{Di} = \alpha\,\eta_i\,q_{Di}^{0}$, with the same $\eta_i$ applied to real and reactive demand at bus $i$. 
...
Notably, the SOCP relaxation is tight for the smallest three systems, but becomes increasingly loose for larger systems, yielding a large gap between the SOCP and chordal SDP relaxations for the largest systems. This ultimately leads to higher false negative rates for the SOCP relaxation, as the neural network trained on the looser SOCP relaxation underestimates the true optimal value function.
...
\toprule
System & Buses & Lines & Gens & \multicolumn{2}{c}{Demand (MW)} \\
\cmidrule(lr){5-6}
 & & & & Nominal & [Min, Max] \\
\midrule
...
\begin{aligned}
    ...
    \text{s.t.} \quad & \mathbf{w}^\top \mathbf{x} \leq b \quad \forall ~ \mathbf{w} \in \mathcal{U}(\bm{\theta}) \\[3pt]
    & \mathbf{x} \in \{0, 1\}^n
\end{aligned}
\label{prob:knapsack}
\end{equation}
This problem admits an exact reformulation as a mixed-integer second-order cone program (MI-SOCP):
\begin{equation}
\begin{aligned}
    v(\bm{\theta}) ~=~ \max_{x} \quad & \mathbf{c}^\top \mathbf{x} \\
    \text{s.t.} \quad & \bm{\mu}^\top \mathbf{x} + \rho\,\|\bm{\Sigma}^{1/2}\mathbf{x}\| \leq b \\[3pt]
    & \mathbf{x} \in \{0,1\}^n
\end{aligned}
...
The chosen convex relaxation in this experiment simply relaxes integer constraints to $\mathbf{x} \in [0, 1]^n$, yielding a second-order cone program (SOCP) that is tractable to solve and provides a tight upper bound on the optimal value of the original problem.

In our experiment, we consider robust 0-1 knapsack with $n=25$ items. The item values $\mathbf{c}$ are drawn once from $\mathrm{Uniform}(20, 30)$, and the budget $b = \tfrac{1}{2}\sum_i c_i$, the robustness parameter $\rho = 1$, and the shape matrix
\begin{align*}
    \bm{\Sigma} ~=~ \tau^2 \big[ (1 - \gamma) \mathbf{I} + \gamma \, \mathbf{1}\mathbf{1}^\top \big], \qquad \tau = 5, ~~ \gamma = 0.8,
\end{align*}
are held constant across all instances, so that the only quantity varying across instances is the ellipsoid center: $\bm{\theta} = \bm{\mu} \in \mathbb{R}^{25}$. Each instance draws $\bm{\mu} \sim \mathcal{N}(\mathbf{c}, \bm{\Sigma})$, centering the nominal weights on the item values so that more valuable items are also heavier on average. The item weights are correlated with each other, with correlation coefficient $\gamma = 0.8$, so that the total weight of the selected items varies substantially across instances.
...
We observe that the greedy heuristic recovers the exact optimal solution on very few instances. Furthermore, the neural network prediction accuracy is much worse for this experiment. The prediction accuracy improves with more training data, and the neural network trained on the continuous relaxation value function yields lower average prediction error and $\hat{q}_{99}$, at the cost of a slightly larger relaxation gap. At our chosen optimality tolerance of $\delta = 5$, we certify up to $70\%$ of solutions as $\delta$-optimal.
...
% Table~\ref{tab:combined} reports every experiment at $\alpha=1\%$ (offset $\hat q_{99}$). Instead of fixing $\delta$ to one relative tolerance across the whole paper, we set it per experiment to whatever "near-optimal" concretely means there: a few percent of $v(\bm\theta)$'s spread for the smooth problems, or for large-scale MIMO detection, comfortably below the cost of flipping one bit of the recovered signal.

% The offline columns are $\bar v_r(\bm\theta)$ (what the network is trained to predict), $\bar v(\bm\theta)$ (the true optimum, where we can compute it), $\bar f(\hat{\mathbf{x}};\bm\theta)$ (the fast solver's value), the network's MAE, and $\hat q_{99}$. We have exact $v(\bm\theta)$ for QCQP ($S$-lemma), inverse kinematics (closed form), both MIMO scales (brute-force maximum-likelihood search over $2^n$ candidates), and robust knapsack (exact MISOCP); those rows show it directly. AC-OPF has no tractable exact solve, so we score it against the tightest relaxation available instead (chordal SDP, or SOCP for \texttt{case2869pegase}, whose chordal SDP is itself intractable) and leave $\bar v(\bm\theta)$ blank. A relaxation value is always a lower bound on the truth, so this substitution can only make the ground-truth event harder to satisfy -- the false-positive counts on those rows are, if anything, conservative.

% False positives are zero or near-zero across every experiment. True positive rates vary a great deal, for a simple reason: IPOPT is strong enough that on several AC-OPF cases, and on QCQP and inverse kinematics, almost every test instance is already $\delta$-optimal, leaving barely any negative class to miss. Large-scale MIMO detection and robust knapsack use weaker local solvers (zero-forcing, greedy) and show a more informative TPR/FPR spread.

% The timing columns show what tighter relaxations cost. On AC-OPF, the chordal SDP is under $3\times$ slower than SOCP on \texttt{case9}, but $19$--$107\times$ slower on \texttt{case118} through \texttt{case89pegase}, and still over $40\times$ slower on \texttt{case1354pegase}. A network prediction, by contrast, takes sub-microsecond time regardless of problem size. The local solver's own timing doesn't scale as cleanly with case size; we don't read much into that given how few instances we timed.

\begin{sidewaystable}[p]
\centering\footnotesize
\caption{Results from all experiments averaged over four folds. We fix $\alpha=1\%$ (offset $\hat q_{99}$) in all experiments, and choose
$\delta$ in each experiment.
The offline columns are $\bar v_r(\bm\theta)$ (relaxed optimal value), $\bar v(\bm\theta)$ (true optimal value, where available), $\bar f(\hat{\mathbf{x}};\bm\theta)$ (the fast solver's objective value), the neural network's mean absolute error (MAE), and the conformal calibration offset $\hat q_{99}$. The online columns are the chosen optimality tolerance $\delta$ of each experiment, as well as the number of true positives (TP), false positives (FP), true negatives (TN), and false negatives (FN) for our certificates. The timing columns show the average time to solve the relaxation and to run the local solver.}
\label{tab:combined}
\resizebox{\linewidth}{!}{%
...
$\bar v_r(\bm\theta)$ & $\bar v(\bm\theta)$ & $\bar f(\hat{\mathbf{x}};\bm\theta)$ & MAE & $q_{99}$ &
$\delta$ & TP & FP & TN & FN &
Relax.\ Solve & Local Solve \\
\midrule
  \multirow{1}{*}{QCQP} & Shor & \texttt{example} & 2.65 & 2.65 & 12.4 & 0.0027 & 0.011 & 0.1 & 2521 & 0 & 2478 & 1 & 0.68 ms & 16.6 ms \\
\midrule
  \multirow{2}{*}{Inverse Kinematics} & Shor & \texttt{planar-arm} & 0.036 & 0.039 & 0.049 & 1.1e-04 & 6.9e-04 & 0.003 & 4548 & 0 & 173 & 273 & 2.88 ms & 148 ms \\
   & Lasserre-2 & \texttt{planar-arm} & 0.039 & 0.039 & 0.049 & 1.7e-04 & 9.6e-04 & 0.003 & 4807 & 0 & 173 & 14 & 164 ms & 75.6 ms \\
\midrule
  \multirow{2}{*}{MIMO Detection} & Shor & \texttt{m=4, n=2} & 1.58 & 1.59 & 1.61 & 0.043 & 0.154 & 0.8 & 4939 & 0 & 44 & 17 & 93.3 ms & 0.23 ms \\
   & Shor & \texttt{m=32, n=16} & 319 & -- & 320 & 30.9 & 93.2 & 160 & 4724 & 0 & 0 & 276 & 5.65 ms & 0.0029 ms \\
\midrule
  \multirow{16}{*}{AC-OPF} & SOCP & \texttt{case9} & 4,692 & -- & 4,692 & 0.42 & 1.49 & 5 & 4994 & 0 & 0 & 6 & 1.80 ms & 4.40 ms \\
   & SOCP & \texttt{case14} & 7,049 & -- & 7,054 & 0.51 & 4.77 & 10 & 1838 & 0 & 0 & 3162 & 0.60 ms & 3.60 ms \\
   & SOCP & \texttt{case39} & 33,520 & -- & 33,530 & 8.02 & 64.0 & 500 & 4651 & 0 & 0 & 0 & 2.40 ms & 20.6 ms \\
   & SOCP & \texttt{case89pegase} & 7,275 & -- & 7,281 & 1.20 & 12.4 & 50 & 4772 & 0 & 0 & 1 & 10.4 ms & 18.7 ms \\
   & SOCP & \texttt{case118} & 113,200 & -- & 113,500 & 17.6 & 200 & 1,000 & 4995 & 0 & 0 & 5 & 6.80 ms & 19.8 ms \\
   & SOCP & \texttt{case300} & 582,700 & -- & 583,800 & 272 & 2,626 & 5,000 & 3862 & 0 & 6 & 74 & 20.0 ms & 41.0 ms \\
   & SOCP & \texttt{case1354pegase} & 67,650 & -- & 67,700 & 3.26 & 15.6 & 100 & 3762 & 0 & 0 & 82 & 128 ms & 247 ms \\
   & SOCP & \texttt{case2869pegase} & 126,500 & -- & 126,800 & 5.68 & 31.1 & 500 & 2957 & 0 & 259 & 0 & 349 ms & 467 ms \\
   & SDP & \texttt{case9} & 4,692 & -- & 4,692 & 0.39 & 1.62 & 5 & 4994 & 0 & 0 & 6 & 2.10 ms & 4.40 ms \\
   & SDP & \texttt{case14} & 7,054 & -- & 7,054 & 0.52 & 4.46 & 10 & 4951 & 0 & 0 & 49 & 5.00 ms & 3.60 ms \\
   & SDP & \texttt{case39} & 33,520 & -- & 33,530 & 7.73 & 87.2 & 500 & 4651 & 0 & 0 & 0 & 27.4 ms & 20.6 ms \\
   & SDP & \texttt{case89pegase} & 7,281 & -- & 7,281 & 1.14 & 12.8 & 50 & 4778 & 0 & 0 & 0 & 1.10 s & 18.7 ms \\
   & SDP & \texttt{case118} & 113,500 & -- & 113,500 & 17.2 & 200 & 1,000 & 5000 & 0 & 0 & 0 & 128 ms & 19.8 ms \\
   & SDP & \texttt{case300} & 583,500 & -- & 583,800 & 199 & 1,365 & 5,000 & 3918 & 0 & 6 & 18 & 787 ms & 41.0 ms \\
   & SDP & \texttt{case1354pegase} & 67,680 & -- & 67,680 & 4.60 & 19.9 & 100 & 3850 & 0 & 0 & 4 & 8.28 s & 247 ms \\
   & SDP & \texttt{case2869pegase} & -- & -- & -- & -- & -- & -- & -- & -- & -- & -- & 40.3 s & 467 ms \\
\midrule
  \multirow{4}{*}{Robust Knapsack} & SOCP & \texttt{N=20000} & 283 & 281 & 272 & 1.31 & 4.22 & 5 & 248 & 0 & 3383 & 1369 & 0.68 ms & 0.069 ms \\
   & SOCP & \texttt{N=80000} & 283 & 281 & 272 & 0.22 & 0.66 & 5 & 1140 & 0 & 3383 & 478 & 0.68 ms & 0.069 ms \\
   & MI-SOCP & \texttt{N=20000} & 281 & 281 & 272 & 1.36 & 4.38 & 5 & 584 & 3 & 3380 & 1032 & 41.0 ms & 0.069 ms \\
   & MI-SOCP & \texttt{N=80000} & 281 & 281 & 272 & 0.63 & 2.21 & 5 & 1140 & 1 & 3382 & 476 & 41.0 ms & 0.069 ms \\
\bottomrule
\end{tabular}%
}
\end{sidewaystable}
...
At our chosen optimality tolerance $\delta$, which varied by experiment, the false positive rate was below the target $1\%$ in every experiment we conducted. The true positive rate varies across experiments, but is generally high when the local solver is strong enough to find near-optimal solutions in most instances (e.g. AC-OPF, QCQP, inverse kinematics), and lower when the local solver is weaker (e.g. large-scale MIMO detection, robust knapsack), as well as when the neural network's conformal offset $\hat{q}_{99}$ is small compared with the optimality tolerance $\delta$. In several experiments (QCQP, inverse kinematics, AC-OPF) the optimality tolerance amounts to only a few percent of the intrinsic spread of the optimal value, while in others (e.g.\ MIMO detection, robust knapsack) it is a substantially larger fraction.

We find that the false negative rate is higher in experiments where the convex relaxation is weaker, which is consistent with our intuition that a large relaxation gap results in an overly conservative certification threshold. In these cases, the neural network's prediction of the relaxed optimal value $v_r(\bm\theta)$ is often substantially lower than the true optimal value $v(\bm\theta)$, so even a candidate solution that is near-globally optimal may not be certified as such. This highlights the importance of using tight convex relaxations to generate training data for our framework.

Furthermore, we find that in some experiments where the value function is highly non-smooth or non-linear (MIMO detection, robust knapsack), increasing the size of the training data improves prediction accuracy and downstream certification accuracy. This result highlights the importance of using a sufficiently large training data set to capture the underlying structure of the value function. However, this also highlights the intrinsic tradeoff between relaxation tightness and training data size: tighter relaxations are often more expensive to solve, which may limit the number of training samples that can be generated in a given time budget (as was the case for the chordal SDP relaxation of AC-OPF for the \texttt{case2869pegase} experiment).

We also note that the probabilistic guarantees of our framework rest on exchangeability between the calibration sample and new test instances. This holds when parameters are drawn i.i.d.\ from a fixed distribution, as in our experiments.
In realistic, time-varying optimization settings, where observed instances of $\bm{\theta}$ may be temporally correlated or subject to distribution shift, the marginal coverage of our framework can degrade, and the calibrated conformal offset should be refreshed online or replaced by a weighted conformal variant (where calibration samples are re-weighted to account for covariate shift)~\citep{tibshirani2019conformal}.

Furthermore, coverage probability in split conformal calibration is marginal: it holds on average over $\Theta$, not conditionally at a particular $\bm{\theta}$, so the error rate may be uneven across the parameter space. Conformalized quantile regression~\citep{romano2019conformalized}, which calibrates a learned prediction error quantile model rather than a single conformal offset, is a way to approximate conditional coverage guarantees and an interesting direction for future work.
...
The architecture ($4$ hidden layers of $64$ neurons) and training budget ($1500$ epochs) used throughout our experiments were selected by ablation on the AC-OPF problem, varying one factor at a time and measuring $4$-fold cross-validated prediction accuracy on the SOCP relaxation, on three test systems spanning two orders of magnitude in size. As certification accuracy depends on the upper tail of the prediction error distribution, we report the $95$th percentile of the absolute error distribution $\hat q_{95}$ as our primary selection criterion, and report the mean absolute percentage error (MAPE) for context. The results are summarized in Tables~\ref{tab:ablation-depth}--\ref{tab:ablation-epochs}.

First, we varied network depth at fixed width of $256$ neurons and training budget of $1000$ epochs (Table~\ref{tab:ablation-depth}). Performance improves with depth, but plateaus or worsens around 4-5 layers for most cases. Next, we varied the training budget for a fixed width network of $256$ neurons and depth 6. We found that performance steadily improved with training budget, but plateaued around $1500$ epochs (Table~\ref{tab:ablation-epochs}). Finally, we varied network width at fixed depth $6$. Width $64$ had the best performance across all experiments, and was substantially cheaper to train than the wider networks. We therefore adopted a neural network architecture of $4$ hidden layers of $64$ neurons, with a training budget of $1500$ epochs, for all of our experiments.

\begin{table}[h]
\centering\footnotesize
\caption{Network-depth ablation on AC-OPF (SOCP relaxation), at fixed width $256$ and $1000$ epochs: $4$-fold cross-validated $95$th-percentile overprediction of $\hat{v}(\bm{\theta})$ above $v_r(\bm{\theta})$ (\%), with mean absolute percent error (MAPE) in parentheses for context. Train time is for \texttt{case1354} (seconds, all four folds).}
\label{tab:ablation-depth}
\begin{tabular}{c ccc r}
\toprule
\textbf{Hidden layers} & \texttt{case9} & \texttt{case118} & \texttt{case1354} & \textbf{Train (s)} \\
\midrule
  $1$ & 1.62 (0.85) & 7.27 (3.64) & 2.45 (1.33) & 1018 \\
  $2$ & 1.25 (0.66) & 3.99 (1.87) & 1.84 (1.18) & 1218 \\
  $3$ & 1.33 (0.70) & 2.82 (1.35) & 0.88 (0.41) & 1868 \\
  $4$ & 1.50 (0.76) & 2.67 (1.36) & 0.90 (0.43) & 1688 \\
  $5$ & 1.50 (0.74) & 2.39 (1.39) & 1.01 (0.46) & 2410 \\
  $6$ & 1.54 (0.79) & 2.86 (1.44) & 1.07 (0.46) & 2095 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering\footnotesize
\caption{Training-budget ablation on AC-OPF (SOCP relaxation, $6\times256$ network): $4$-fold cross-validated $95$th-percentile overprediction of $\hat{v}(\bm{\theta})$ above $v_r(\bm{\theta})$ (\%), with mean absolute percent error (MAPE) in parentheses for context. Train time is for \texttt{case1354} (seconds, all four folds).}
\label{tab:ablation-epochs}
\begin{tabular}{c ccc r}
\toprule
\textbf{Epochs} & \texttt{case9} & \texttt{case118} & \texttt{case1354} & \textbf{Train (s)} \\
\midrule
  $100$ & 3.14 (1.62) & 11.64 (4.55) & 2.32 (1.18) & 204 \\
  $200$ & 2.41 (1.23) & 8.97 (3.33) & 1.54 (0.77) & 401 \\
  $500$ & 1.92 (0.99) & 4.75 (2.00) & 1.17 (0.55) & 1053 \\
  $1000$ & 1.54 (0.79) & 2.86 (1.44) & 1.07 (0.46) & 2095 \\
  $1500$ & 1.31 (0.65) & 2.07 (1.27) & 0.94 (0.43) & 3387 \\
  $2000$ & 1.31 (0.62) & 2.11 (1.25) & 0.89 (0.42) & 4576 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[h]
\centering\footnotesize
\caption{Layer-width ablation on AC-OPF (SOCP relaxation), at fixed depth $6$ and $1000$ epochs: $4$-fold cross-validated $95$th-percentile overprediction of $\hat{v}(\bm{\theta})$ above $v_r(\bm{\theta})$ (\%), with mean absolute percent error (MAPE) in parentheses for context. Train time is for \texttt{case1354} (seconds, all four folds).}
\label{tab:ablation-width}
\begin{tabular}{c ccc r}
\toprule
\textbf{Layer width} & \texttt{case9} & \texttt{case118} & \texttt{case1354} & \textbf{Train (s)} \\
\midrule
  $64$ & 1.22 (0.61) & 1.78 (1.14) & 0.98 (0.45) & 1214 \\
  $128$ & 1.46 (0.73) & 2.12 (1.25) & 1.04 (0.45) & 1502 \\
  $256$ & 1.54 (0.79) & 2.86 (1.44) & 1.07 (0.46) & 2095 \\
  $512$ & 1.51 (0.75) & 2.99 (1.58) & 1.02 (0.47) & 3049 \\
\bottomrule
\end{tabular}
\end{table}
...
% \begin{table}[h]
% \centering\footnotesize
% \caption{Network-\emph{depth} ablation at the \emph{adopted} width $64$ (SOCP relaxation, $1000$ epochs): $4$-fold cross-validated $95$th-percentile overprediction of $\hat{v}$ above $v_r$ (\%), MAPE in parentheses. Depth $5$--$6$ edge out depth $4$ on this tail metric on \texttt{case118} ($1.74$/$1.78$ vs.\ $2.07$); we adopt depth $4$ regardless as a practical plateau point with roughly $30$--$40\%$ fewer parameters and comparable training cost, not as the strict per-case optimum. Train time is for \texttt{case1354} (seconds, all four folds).}
% \label{tab:ablation-depth-w64}
% \begin{tabular}{c ccc r}
% \toprule
% \textbf{Hidden layers} & \texttt{case9} & \texttt{case118} & \texttt{case1354} & \textbf{Train (s)} \\
% \midrule
%   $1$ & 2.75 (1.53) & 6.04 (3.55) & 2.75 (1.60) & 872 \\
%   $2$ & 1.88 (0.97) & 2.70 (1.56) & 1.15 (0.61) & 940 \\
%   $3$ & 1.31 (0.67) & 2.45 (1.37) & 0.81 (0.42) & 982 \\
%   $4$ & 1.16 (0.57) & 2.07 (1.19) & 0.82 (0.40) & 1141 \\
%   $5$ & 1.24 (0.62) & 1.74 (1.14) & 0.84 (0.39) & 1136 \\
%   $6$ & 1.22 (0.61) & 1.78 (1.14) & 0.98 (0.45) & 1214 \\
% \bottomrule
% \end{tabular}
% \end{table}

% \subsection{Robust Knapsack: Data-Size Ablation}

% Unlike the smooth-in-$\bm\theta$ problems above, robust knapsack's optimal value has genuine
% argmax-switching discontinuities in $\bm\theta$ (Section~\ref{sec:discussion}), so we separately
% ablate whether more training data closes the resulting certification-accuracy gap, for the
% correlated-$\bm\mu$ redesign (\S\ref{subsec:results}) at the adopted $4\times64$ architecture.
% Table~\ref{tab:knapsack-ablation} reports the effective slack $(\hat q_{99} + \text{relax.\ gap})
% /\mathrm{std}(v^\ast)$ -- the calibrated conformal offset, plus the SOCP-relaxation target's fixed
% gap to $v^\ast$ where applicable (zero for the exact-MISOCP target), as a fraction of the optimal
% value's spread; this is the quantity that determines how tight a certification threshold $\delta$
% can be -- at $20$k and $80$k training instances, for both targets. Unlike an earlier pass of this
% ablation at the wider $6\times256$ architecture (which found a data-insensitive floor), $4\times$
% more data reduces this effective slack substantially for both targets here (roughly $50\%$ for
% exact-MISOCP, $60\%$ for SOCP-relaxation), consistent with the earlier floor having been a
% capacity/tail-fit artifact of that wider network rather than a fundamental limit imposed by the
% discontinuities themselves. The SOCP-relaxation target improves faster with data -- its smoother
% learning signal is more data-hungry, and even its fixed relaxation gap does not fully offset the
% gain -- and the two targets are nearly tied at $80$k ($0.051$ vs.\ $0.047$), whereas exact-MISOCP
% was clearly ahead at $20$k ($0.093$ vs.\ $0.126$).

% \begin{table}[h]
% \centering\footnotesize
% \caption{Robust knapsack (correlated-$\bm\mu$), data-size ablation at the adopted $4\times64$
% architecture: $4$-fold cross-validated effective slack $(\hat q_{99} + \text{relax.\ gap})
% /\mathrm{std}(v^\ast)$ for both targets, at $20$k and $80$k training instances (relax.\ gap is
% $0$ for the exact-MISOCP target).}
% \label{tab:knapsack-ablation}
% \begin{tabular}{c c c}
% \toprule
% \textbf{Training instances} & \textbf{Target} & Effective slack $/\mathrm{std}(v^\ast)$ \\
% \midrule
%   $20$k & Exact-MISOCP & $0.0934$ \\
%   $80$k & Exact-MISOCP & $0.0470$ \\
% \midrule
%   $20$k & SOCP-relaxation & $0.1264$ \\
%   $80$k & SOCP-relaxation & $0.0506$ \\
% \bottomrule
% \end{tabular}
% \end{table}
